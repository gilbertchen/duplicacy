...
func (storage *GCDStorage) shouldRetry(threadIndex int, err error) (bool, error) {
	...
	if storage.attempts[threadIndex] >= MAX_ATTEMPTS {
		util.LOG_INFO("GCD_RETRY", "[%d] Maximum number of retries reached (backoff: %d, attempts: %d)",
	...
	util.LOG_DEBUG("GCD_RETRY", "[%d] %s; retrying after %.2f seconds (backoff: %d, attempts: %d)",
...
func CreateGCDStorage(tokenFile string, driveID string, storagePath string, threads int) (storage *GCDStorage, err error) {
	...
	if isServiceAccount && !strings.Contains(storagePath, "/") {
		...
		if err != nil {
			util.LOG_WARN("GCD_STORAGE", "Failed to check if %s is a shared folder: %v", storagePath, err)
		}
	}
		...
		for len(parents) > 0 || activeWorkers > 0 {
			...
			if len(parents) > 0 && activeWorkers < storage.numberOfThreads {
				...
				go func(parent string) {
					...
					util.LOG_DEBUG("GCD_STORAGE", "Listing %s; %d items returned", parent, len(entries))
...
func (storage *GCDStorage) DeleteFile(threadIndex int, filePath string) (err error) {
	...
	if err != nil {
		util.LOG_TRACE("GCD_STORAGE", "Ignored file deletion error: %v", err)
		return nil
	}
	...
	for {
		...
		if retry, err := storage.shouldRetry(threadIndex, err); err == nil && !retry {
			...
			if e, ok := err.(*googleapi.Error); ok && e.Code == 404 {
				util.LOG_TRACE("GCD_STORAGE", "File %s has disappeared before deletion", filePath)
				return nil
			}
			return err
		}
...
func (storage *GCDStorage) DownloadFile(threadIndex int, filePath string, chunk io.Writer) (err error) {
	...
	for {
		...
		if e, ok := err.(*googleapi.Error); ok {
			if strings.Contains(err.Error(), "cannotDownloadAbusiveFile") || len(e.Errors) > 0 && e.Errors[0].Reason == "cannotDownloadAbusiveFile" {
				util.LOG_WARN("GCD_STORAGE", "%s is marked as abusive, will download anyway.", filePath)
	...
	_, err = util.RateLimitedCopy(chunk, response.Body, storage.DownloadRateLimit/storage.numberOfThreads)
	return err
}
...
func (storage *GCDStorage) UploadFile(threadIndex int, filePath string, content []byte) (err error) {
	...
	for {
		reader := util.CreateRateLimitedReader(content, storage.UploadRateLimit/storage.numberOfThreads)
