...
func (client *OneDriveClient) call(url string, method string, input interface{}, contentType string) (io.ReadCloser, int64, error) {
	...
	for i := 0; i < 12; i++ {
		...
		util.LOG_DEBUG("ONEDRIVE_CALL", "%s %s", method, url)
		...
		case *util.RateLimitedReader:
			input.(*util.RateLimitedReader).Reset()
			inputReader = input.(*util.RateLimitedReader)
		}
		...
		if reader, ok := inputReader.(*util.RateLimitedReader); ok {
		...
		if err != nil {
			if client.IsConnected {
				if strings.Contains(err.Error(), "TLS handshake timeout") {
					...
					util.LOG_INFO("ONEDRIVE_RETRY", "TLS handshake timeout; retry after %d milliseconds", retryAfter)
					...
					util.LOG_INFO("ONEDRIVE_RETRY", "%v; retry after %d milliseconds", err, retryAfter)
					time.Sleep(retryAfter * time.Millisecond)
				}
				backoff *= 2
				if backoff > 256 {
		...
		if response.StatusCode == 401 {
			...
			util.LOG_INFO("ONEDRIVE_RETRY", "Response code: %d; retry after %d milliseconds", response.StatusCode, delay)
...
func (client *OneDriveClient) UploadFile(path string, content []byte, rateLimit int) (err error) {
	...
	if !client.IsBusiness || (client.TestMode && rand.Int()%2 == 0) {
		...
		readCloser, _, err := client.call(url, "PUT", util.CreateRateLimitedReader(content, rateLimit), "application/octet-stream")
		if err != nil {
			return err
		}
...
func (client *OneDriveClient) UploadFileSession(uploadURL string, content []byte, rateLimit int) (err error) {
	...
	readCloser, _, err := client.call(uploadURL, "PUT", util.CreateRateLimitedReader(content, rateLimit), "")
	if err != nil {
		return err
	}
...
func (client *OneDriveClient) CreateDirectory(path string, name string) error {
	...
	if err != nil {
		if e, ok := err.(OneDriveError); ok && e.Status == 409 {
			...
			util.LOG_TRACE("ONEDRIVE_MKDIR", "The directory '%s/%s' already exists", path, name)
			return nil
		}
		return err
	}
