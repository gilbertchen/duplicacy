...
func (storage *AzureStorage) DownloadFile(threadIndex int, filePath string, chunk io.Writer) (err error) {
	...
	_, err = util.RateLimitedCopy(chunk, readCloser, storage.DownloadRateLimit/len(storage.containers))
	return err
}
...
func (storage *AzureStorage) UploadFile(threadIndex int, filePath string, content []byte) (err error) {
	...
	for {
		reader := util.CreateRateLimitedReader(content, storage.UploadRateLimit/len(storage.containers))
		blob := storage.containers[threadIndex].GetBlobReference(filePath)
		...
		util.LOG_INFO("AZURE_RETRY", "Connection unexpectedly terminated: %v; retrying", err)
		tries++
	}
