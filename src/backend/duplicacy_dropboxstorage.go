...
func (storage *DropboxStorage) DownloadFile(threadIndex int, filePath string, chunk io.Writer) (err error) {
	...
	_, err = util.RateLimitedCopy(chunk, output.Body, storage.DownloadRateLimit/len(storage.clients))
	return err
...
func (storage *DropboxStorage) UploadFile(threadIndex int, filePath string, content []byte) (err error) {
	...
	input := &dropbox.UploadInput{
		...
		AutoRename: false,
		Mute:       true,
		Reader:     util.CreateRateLimitedReader(content, storage.UploadRateLimit/len(storage.clients)),
	}
