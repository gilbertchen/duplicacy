...
func (storage *SwiftStorage) DownloadFile(threadIndex int, filePath string, chunk io.Writer) (err error) {

	file, _, err := storage.connection.ObjectOpen(storage.ctx, storage.container, storage.storageDir+filePath, false, nil)
	if err != nil {
		return err
	}
	_, err = util.RateLimitedCopy(chunk, file, storage.DownloadRateLimit/storage.threads)
	return err
}

// UploadFile writes 'content' to the file at 'filePath'.
func (storage *SwiftStorage) UploadFile(threadIndex int, filePath string, content []byte) (err error) {
	reader := util.CreateRateLimitedReader(content, storage.UploadRateLimit/storage.threads)
