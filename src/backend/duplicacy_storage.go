...
type ConfigI interface {
	IsFixedNesting() bool
	NestingFile(filePath string, DerivedStorage Storage) ([]byte, error)
}
...
type Storage interface {
	...
	DownloadFile(threadIndex int, filePath string, chunk io.Writer) (err error)
	...
	SetNestingLevels(config ConfigI)
...
func (storage *StorageBase) SetNestingLevels(config ConfigI) {

	// 'FixedNesting' is true only for the 'config' file with the new format (2.0.10+)
	if config.IsFixedNesting() {
		...
		if err == nil && exist {
			...
			if nestingFile_GetBytes, err := config.NestingFile("nesting", storage.DerivedStorage); err == nil {
				var nesting struct {
					ReadLevels []int `json:"read-levels"`
					WriteLevel int   `json:"write-level"`
				}
				if json.Unmarshal(nestingFile_GetBytes, &nesting) == nil {
	...
	util.LOG_DEBUG("STORAGE_NESTING", "Chunk read levels: %v, write level: %d", storage.readLevels, storage.writeLevel)
	for _, level := range storage.readLevels {
		if storage.writeLevel == level {
			return
		}
	}
	util.LOG_ERROR("STORAGE_NESTING", "The write level %d isn't in the read levels %v", storage.readLevels, storage.writeLevel)
}
	...
	for i, line := range strings.Split(string(content), "\n") {
		...
		if matched[1] == remote.String() {
			if keyString != matched[2] {
				util.LOG_WARN("HOSTKEY_OLD", "The existing key for '%s' is %s (file %s, line %d)",
					remote.String(), matched[2], hostFile, i)
				util.LOG_WARN("HOSTKEY_NEW", "The new key is '%s'", keyString)
...
func CreateStorage(preference util.Preference, resetPassword bool, threads int) (storage Storage) {
	...
	if isFileStorage {
		...
		if err != nil {
			util.LOG_ERROR("STORAGE_CREATE", "Failed to load the file storage at %s: %v", storageURL, err)
			return nil
		}
		return fileStorage
	}
	...
	if matched == nil {
		util.LOG_ERROR("STORAGE_CREATE", "Unrecognizable storage URL: %s", storageURL)
		return nil
		...
		if storageDir == "" {
			util.LOG_ERROR("STORAGE_CREATE", "The SFTP storage directory can't be empty")
			return nil
		}
		...
		keyFile := util.GetPasswordFromPreference(preference, "ssh_key_file")
		passphrase := ""

		password := ""
		passwordCallback := func() (string, error) {
			util.LOG_DEBUG("SSH_PASSWORD", "Attempting password login")
			password = util.GetPassword(preference, "ssh_password", "Enter SSH password:", false, resetPassword)
			return password, nil
		}
		...
		keyboardInteractive := func(user, instruction string, questions []string, echos []bool) (answers []string,
			err error) {
			if len(questions) == 1 {
				util.LOG_DEBUG("SSH_INTERACTIVE", "Attempting keyboard interactive login")
				password = util.GetPassword(preference, "ssh_password", "Enter SSH password:", false, resetPassword)
		...
		publicKeysCallback := func() ([]ssh.Signer, error) {
			util.LOG_DEBUG("SSH_PUBLICKEY", "Attempting public key authentication")
			...
			if agentSock != "" {
				...
				if err == nil {
					util.LOG_DEBUG("SSH_AGENT", "Attempting public key authentication via agent")
					sshAgent := agent.NewClient(connection)
					signers, err = sshAgent.Signers()
					if err != nil {
						util.LOG_DEBUG("SSH_AGENT", "Can't log in using public key authentication via agent: %v", err)
					} else if len(signers) == 0 {
						util.LOG_DEBUG("SSH_AGENT", "SSH agent doesn't return any signer")
					}
				}
			}

			keyFile = util.GetPassword(preference, "ssh_key_file", "Enter the path of the private key file:",
				true, resetPassword)
			...
			if keyFile == "" {
				util.LOG_INFO("SSH_PUBLICKEY", "No private key file is provided")
			} else {
				var content []byte
				content, err = ioutil.ReadFile(keyFile)
				if err != nil {
					util.LOG_INFO("SSH_PUBLICKEY", "Failed to read the private key file: %v", err)
				} else {
					keySigner, err = ssh.ParsePrivateKey(content)
					if err != nil {
						if _, ok := err.(*ssh.PassphraseMissingError); ok {
							util.LOG_TRACE("SSH_PUBLICKEY", "The private key file is encrypted")
							passphrase = util.GetPassword(preference, "ssh_passphrase", "Enter the passphrase to decrypt the private key file:", false, resetPassword)
							if len(passphrase) == 0 {
								util.LOG_INFO("SSH_PUBLICKEY", "No passphrase to descrypt the private key file %s", keyFile)
							} else {
								keySigner, err = ssh.ParsePrivateKeyWithPassphrase(content, []byte(passphrase))
								if err != nil {
									util.LOG_INFO("SSH_PUBLICKEY", "Failed to parse the encrypted private key file %s: %v", keyFile, err)
								}
							}
						} else {
							util.LOG_INFO("SSH_PUBLICKEY", "Failed to parse the private key file %s: %v", keyFile, err)
						}
					}
					...
					if keySigner != nil {
						...
						if stat, err := os.Stat(certFile); err == nil && !stat.IsDir() {
							util.LOG_DEBUG("SSH_CERTIFICATE", "Attempting to use ssh certificate from file %s", certFile)
							var content []byte
							content, err = ioutil.ReadFile(certFile)
							if err != nil {
								util.LOG_INFO("SSH_CERTIFICATE", "Failed to read ssh certificate file %s: %v", certFile, err)
							} else {
								pubKey, _, _, _, err := ssh.ParseAuthorizedKey(content)
								if err != nil {
									util.LOG_INFO("SSH_CERTIFICATE", "Failed parse ssh certificate file %s: %v", certFile, err)
								} else {
									certSigner, err := ssh.NewCertSigner(pubKey.(*ssh.Certificate), keySigner)
									if err != nil {
										util.LOG_INFO("SSH_CERTIFICATE", "Failed to create certificate signer: %v", err)
									} else {
		...
		if util.RunInBackground {
			...
			authMethods = []ssh.AuthMethod{}
			if util.KeyringGet(passwordKey) != "" {
			...
			if util.KeyringGet(keyFileKey) != "" || os.Getenv("SSH_AUTH_SOCK") != "" {
		...
		if err != nil {
			util.LOG_ERROR("STORAGE_CREATE", "Failed to load the SFTP storage at %s: %v", storageURL, err)
			return nil
		}

		if keyFile != "" {
			util.SavePassword(preference, "ssh_key_file", keyFile)
			if passphrase != "" {
				util.SavePassword(preference, "ssh_passphrase", passphrase)
			}
		} else if password != "" {
			util.SavePassword(preference, "ssh_password", password)
		}
		return sftpStorage
		...
		accessKey := util.GetPassword(preference, "s3_id", "Enter S3 Access Key ID:", true, resetPassword)
		secretKey := util.GetPassword(preference, "s3_secret", "Enter S3 Secret Access Key:", true, resetPassword)
		...
		if matched[1] == "s3c" {
			...
			if err != nil {
				util.LOG_ERROR("STORAGE_CREATE", "Failed to load the S3C storage at %s: %v", storageURL, err)
				return nil
			}
		} else {
			...
			if err != nil {
				util.LOG_ERROR("STORAGE_CREATE", "Failed to load the S3 storage at %s: %v", storageURL, err)
				return nil
			}
		}
		util.SavePassword(preference, "s3_id", accessKey)
		util.SavePassword(preference, "s3_secret", secretKey)
		...
		key := util.GetPassword(preference, "wasabi_key",
			"Enter Wasabi key:", true, resetPassword)
		secret := util.GetPassword(preference, "wasabi_secret",
		...
		if err != nil {
			util.LOG_ERROR("STORAGE_CREATE", "Failed to load the Wasabi storage at %s: %v", storageURL, err)
			return nil
		}

		util.SavePassword(preference, "wasabi_key", key)
		util.SavePassword(preference, "wasabi_secret", secret)

		return storage

	} else if matched[1] == "dropbox" {
		storageDir := matched[3] + matched[5]
		token := util.GetPassword(preference, "dropbox_token", "Enter Dropbox refresh token:", true, resetPassword)
		dropboxStorage, err := CreateDropboxStorage(token, storageDir, 1, threads)
		if err != nil {
			util.LOG_ERROR("STORAGE_CREATE", "Failed to load the dropbox storage: %v", err)
			return nil
		}
		util.SavePassword(preference, "dropbox_token", token)
		return dropboxStorage
	} else if matched[1] == "b2" {
		bucket := matched[3]
		storageDir := matched[5]

		accountID := util.GetPassword(preference, "b2_id", "Enter Backblaze account or application id:", true, resetPassword)
		applicationKey := util.GetPassword(preference, "b2_key", "Enter corresponding Backblaze application key:", true, resetPassword)

		b2Storage, err := CreateB2Storage(accountID, applicationKey, "", bucket, storageDir, threads)
		if err != nil {
			util.LOG_ERROR("STORAGE_CREATE", "Failed to load the Backblaze B2 storage at %s: %v", storageURL, err)
			return nil
		}
		util.SavePassword(preference, "b2_id", accountID)
		util.SavePassword(preference, "b2_key", applicationKey)
		return b2Storage
		...
		if container == "" {
			util.LOG_ERROR("STORAGE_CREATE", "The container name for the Azure storage can't be empty")
			return nil
		}

		prompt := fmt.Sprintf("Enter the Access Key for the Azure storage account %s:", account)
		accessKey := util.GetPassword(preference, "azure_key", prompt, true, resetPassword)

		azureStorage, err := CreateAzureStorage(account, accessKey, container, threads)
		if err != nil {
			util.LOG_ERROR("STORAGE_CREATE", "Failed to load the Azure storage at %s: %v", storageURL, err)
			return nil
		}
		util.SavePassword(preference, "azure_key", accessKey)
		return azureStorage
		...
		tokenFile := util.GetPassword(preference, "acd_token", prompt, true, resetPassword)
		acdStorage, err := CreateACDStorage(tokenFile, storagePath, threads)
		if err != nil {
			util.LOG_ERROR("STORAGE_CREATE", "Failed to load the Amazon Cloud Drive storage at %s: %v", storageURL, err)
			return nil
		}
		util.SavePassword(preference, "acd_token", tokenFile)
		return acdStorage
		...
		tokenFile := util.GetPassword(preference, "gcs_token", prompt, true, resetPassword)
		gcsStorage, err := CreateGCSStorage(tokenFile, bucket, storageDir, threads)
		if err != nil {
			util.LOG_ERROR("STORAGE_CREATE", "Failed to load the Google Cloud Storage backend at %s: %v", storageURL, err)
			return nil
		}
		util.SavePassword(preference, "gcs_token", tokenFile)
		return gcsStorage
		...
		tokenFile := util.GetPassword(preference, "gcd_token", prompt, true, resetPassword)
		gcdStorage, err := CreateGCDStorage(tokenFile, driveID, storagePath, threads)
		if err != nil {
			util.LOG_ERROR("STORAGE_CREATE", "Failed to load the Google Drive storage at %s: %v", storageURL, err)
			return nil
		}
		util.SavePassword(preference, "gcd_token", tokenFile)
		return gcdStorage
		...
		tokenFile := util.GetPassword(preference, matched[1]+"_token", prompt, true, resetPassword)
		...
		client_id := util.GetPasswordFromPreference(preference, matched[1]+"_client_id")
		client_secret := ""
		...
		if client_id != "" {
			...
			client_secret = util.GetPassword(preference, matched[1]+"_client_secret", prompt, true, resetPassword)
		}

		oneDriveStorage, err := CreateOneDriveStorage(tokenFile, matched[1] == "odb", storagePath, threads, client_id, client_secret, drive_id)
		if err != nil {
			util.LOG_ERROR("STORAGE_CREATE", "Failed to load the OneDrive storage at %s: %v", storageURL, err)
			return nil
		}

		util.SavePassword(preference, matched[1]+"_token", tokenFile)
		if client_id != "" {
			util.SavePassword(preference, matched[1]+"_client_secret", client_secret)
		}
		...
		tokenFile := util.GetPassword(preference, "hubic_token", prompt, true, resetPassword)
		hubicStorage, err := CreateHubicStorage(tokenFile, storagePath, threads)
		if err != nil {
			util.LOG_ERROR("STORAGE_CREATE", "Failed to load the Hubic storage at %s: %v", storageURL, err)
			return nil
		}
		util.SavePassword(preference, "hubic_token", tokenFile)
		return hubicStorage
	} else if matched[1] == "swift" {
		prompt := fmt.Sprintf("Enter the OpenStack Swift key:")
		key := util.GetPassword(preference, "swift_key", prompt, true, resetPassword)
		swiftStorage, err := CreateSwiftStorage(storageURL[8:], key, threads)
		if err != nil {
			util.LOG_ERROR("STORAGE_CREATE", "Failed to load the OpenStack Swift storage at %s: %v", storageURL, err)
			return nil
		}
		util.SavePassword(preference, "swift_key", key)
		return swiftStorage
	} else if matched[1] == "webdav" || matched[1] == "webdav-http" {
		server := matched[3]
		username := matched[2]
		if username == "" {
			util.LOG_ERROR("STORAGE_CREATE", "No username is provided to access the WebDAV storage")
			return nil
		}
		username = username[:len(username)-1]
		storageDir := matched[5]
		...
		password := util.GetPassword(preference, "webdav_password", prompt, true, resetPassword)
		webDAVStorage, err := CreateWebDAVStorage(server, port, username, password, storageDir, useHTTP, threads)
		if err != nil {
			util.LOG_ERROR("STORAGE_CREATE", "Failed to load the WebDAV storage at %s: %v", storageURL, err)
			return nil
		}
		util.SavePassword(preference, "webdav_password", password)
		return webDAVStorage
	} else if matched[1] == "fabric" {
		endpoint := matched[3]
		storageDir := matched[5]
		prompt := fmt.Sprintf("Enter the token for accessing the Storage Made Easy File Fabric storage:")
		token := util.GetPassword(preference, "fabric_token", prompt, true, resetPassword)
		smeStorage, err := CreateFileFabricStorage(endpoint, token, storageDir, threads)
		if err != nil {
			util.LOG_ERROR("STORAGE_CREATE", "Failed to load the File Fabric storage at %s: %v", storageURL, err)
			return nil
		}
		util.SavePassword(preference, "fabric_token", token)
		return smeStorage
		...
		apiKey := util.GetPassword(preference, "storj_key", "Enter the API access key:", true, resetPassword)
		passphrase := util.GetPassword(preference, "storj_passphrase", "Enter the passphrase:", true, resetPassword)
		storjStorage, err := CreateStorjStorage(satellite, apiKey, passphrase, bucket, storageDir, threads)
		if err != nil {
			util.LOG_ERROR("STORAGE_CREATE", "Failed to load the Storj storage at %s: %v", storageURL, err)
			return nil
		}
		util.SavePassword(preference, "storj_key", apiKey)
		util.SavePassword(preference, "storj_passphrase", passphrase)
		return storjStorage
	} else if matched[1] == "smb" {
		server := matched[3]
		username := matched[2]
		if username == "" {
			util.LOG_ERROR("STORAGE_CREATE", "No username is provided to access the SAMBA storage")
			return nil
		}
		username = username[:len(username)-1]
		storageDir := matched[5]
		...
		if !strings.Contains(storageDir, "/") {
			util.LOG_ERROR("STORAGE_CREATE", "No share name specified for the SAMBA storage")
			return nil
		}
		...
		password := util.GetPassword(preference, "smb_password", prompt, true, resetPassword)
		sambaStorage, err := CreateSambaStorage(server, port, username, password, shareName, storageDir, threads)
		if err != nil {
			util.LOG_ERROR("STORAGE_CREATE", "Failed to load the SAMBA storage at %s: %v", storageURL, err)
			return nil
		}
		util.SavePassword(preference, "smb_password", password)
		return sambaStorage

	} else {
		util.LOG_ERROR("STORAGE_CREATE", "The storage type '%s' is not supported", matched[1])
		return nil
	}
