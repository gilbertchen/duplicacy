...
func (client *HubicClient) call(url string, method string, input interface{}, extraHeader map[string]string) (io.ReadCloser, int64, string, error) {
	...
	for i := 0; i < 11; i++ {
		...
		util.LOG_DEBUG("HUBIC_CALL", "%s %s", method, url)
		...
		if err != nil {
			if url != HubicCredentialURL {
				...
				util.LOG_INFO("HUBIC_CALL", "%s %s returned an error: %v; retry after %d milliseconds", method, url, err, retryAfter)
		...
		if response.StatusCode == 401 {
			...
			retryAfter := time.Duration((0.5 + rand.Float32()) * 1000.0 * float32(backoff))
			util.LOG_INFO("HUBIC_RETRY", "Response status: %d; retry after %d milliseconds", response.StatusCode, retryAfter)
			time.Sleep(retryAfter * time.Millisecond)
			backoff *= 2
			continue
...
func (client *HubicClient) UploadFile(path string, content []byte, rateLimit int) (err error) {
	...
	readCloser, _, _, err := client.call(url, "PUT", util.CreateRateLimitedReader(content, rateLimit), header)
