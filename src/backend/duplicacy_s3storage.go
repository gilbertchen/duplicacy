...
func (storage *S3Storage) DownloadFile(threadIndex int, filePath string, chunk io.Writer) (err error) {
	...
	_, err = util.RateLimitedCopy(chunk, output.Body, storage.DownloadRateLimit/storage.numberOfThreads)
	return err
...
func (storage *S3Storage) UploadFile(threadIndex int, filePath string, content []byte) (err error) {
	...
	for {
		input := &s3.PutObjectInput{
			...
			Body:        util.CreateRateLimitedReader(content, storage.UploadRateLimit/storage.numberOfThreads),
		...
		util.LOG_INFO("S3_RETRY", "Retrying on %s: %v", reflect.TypeOf(err), err)
		attempts += 1
	}
}
