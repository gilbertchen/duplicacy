...
func (storage *SFTPStorage) retry(f func() error) error {
	...
	for i := 0; ; i++ {
		...
		if err != nil && strings.Contains(err.Error(), "EOF") && i < storage.numberOfTries {
			util.LOG_WARN("SFTP_RETRY", "Encountered an error (%v); retry after %d second(s)", err, delay/time.Second)
			time.Sleep(delay)
			delay *= 2
			...
			if err != nil {
				util.LOG_WARN("SFTP_RECONNECT", "Failed to connect to %s: %v; retrying", storage.serverAddress, err)
			...
			client, err := sftp.NewClient(connection)
			if err != nil {
				util.LOG_WARN("SFTP_RECONNECT", "Failed to create a new SFTP client to %s: %v; retrying", storage.serverAddress, err)
				connection.Close()
				storage.clientLock.Unlock()
				continue
			}
...
func (storage *SFTPStorage) DeleteFile(threadIndex int, filePath string) (err error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			util.LOG_TRACE("SFTP_STORAGE", "File %s has disappeared before deletion", filePath)
			return nil
		}
		return err
	}
...
func (storage *SFTPStorage) DownloadFile(threadIndex int, filePath string, chunk io.Writer) (err error) {
	return storage.retry(func() error {
		...
		defer file.Close()
		if _, err = util.RateLimitedCopy(chunk, file, storage.DownloadRateLimit/storage.numberOfThreads); err != nil {
			return err
		}
		return nil
	})
