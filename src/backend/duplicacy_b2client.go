...
func NewB2Client(applicationKeyID string, applicationKey string, downloadURL string, storageDir string, threads int) *B2Client {
	...
	if value, found := os.LookupEnv("DUPLICACY_B2_RETRIES"); found && value != "" {
		...
		util.LOG_INFO("B2_RETRIES", "Setting maximum retries for B2 to %d", maximumRetries)
	}
...
func (client *B2Client) call(threadIndex int, requestURL string, method string, requestHeaders map[string]string, input interface{}) (
	...
	for {
		...
		case *util.RateLimitedReader:
			isUpload = true
			rateLimitedReader := input.(*util.RateLimitedReader)
		...
		if err != nil {
			...
			util.LOG_TRACE("BACKBLAZE_CALL", "[%d] URL request '%s' returned an error: %v", threadIndex, requestURL, err)
		...
		if err := json.NewDecoder(response.Body).Decode(e); err != nil {
			util.LOG_TRACE("BACKBLAZE_CALL", "[%d] URL request '%s %s' returned status code %d", threadIndex, method, requestURL, response.StatusCode)
		} else {
			util.LOG_TRACE("BACKBLAZE_CALL", "[%d] URL request '%s %s' returned %d %s", threadIndex, method, requestURL, response.StatusCode, e.Message)
		}
...
func (client *B2Client) AuthorizeAccount(threadIndex int) (err error, allowed bool) {
	...
	util.LOG_INFO("BACKBLAZE_URL", "Download URL is: %s", client.DownloadURL)
	...
	for {
		...
		if singleFile && !includeVersions {
			if responseHeader == nil {
				util.LOG_DEBUG("BACKBLAZE_LIST", "%s did not return headers", apiURL)
				return []*B2Entry{}, nil
			}
			...
			if err == nil {
				...
				util.LOG_WARN("BACKBLAZE_UNESCAPE", "Failed to unescape the file name %s", fileName)
			}
			fileAction := "upload"
...
func (client *B2Client) UploadFile(threadIndex int, filePath string, content []byte, rateLimit int) (err error) {
	...
	readCloser, _, _, err := client.call(threadIndex, "", http.MethodPost, headers, util.CreateRateLimitedReader(content, rateLimit))
	if err != nil {
		return err
	}
