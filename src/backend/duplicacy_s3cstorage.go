...
func (storage *S3CStorage) DownloadFile(threadIndex int, filePath string, chunk io.Writer) (err error) {
	...
	_, err = util.RateLimitedCopy(chunk, readCloser, storage.DownloadRateLimit/len(storage.buckets))
	return err
...
func (storage *S3CStorage) UploadFile(threadIndex int, filePath string, content []byte) (err error) {
	...
	options := s3.Options{}
	reader := util.CreateRateLimitedReader(content, storage.UploadRateLimit/len(storage.buckets))
