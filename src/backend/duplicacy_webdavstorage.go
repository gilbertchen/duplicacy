	...
	for i := 0; i < 8; i++ {
		...
		if method == "PROPFIND" {
			...
			if storage.UploadRateLimit <= 0 {
				...
				dataReader = util.CreateRateLimitedReader(data, storage.UploadRateLimit/storage.threads)
			}
		} else if method == "MOVE" {
		...
		//util.LOG_INFO("debug", "Request: %s", requestDump)

		response, err := storage.client.Do(request)
		if err != nil {
			util.LOG_TRACE("WEBDAV_ERROR", "URL request '%s %s' returned an error (%v)", method, uri, err)
		...
		util.LOG_INFO("WEBDAV_RETRY", "URL request '%s %s' returned status code %d", method, uri, response.StatusCode)
		backoff = storage.retry(backoff)
	}
	...
	for tries := 0; ; tries++ {
		...
		if err != nil {
			if strings.Contains(err.Error(), "unexpected EOF") && tries < maxTries {
				util.LOG_WARN("WEBDAV_RETRY", "Retrying on %v", err)
				continue
			}
			return nil, err
		}
...
func (storage *WebDAVStorage) CreateDirectory(threadIndex int, dir string) (err error) {
	...
	if err != nil {
		if err == errWebDAVMethodNotAllowed || err == errWebDAVMovedPermanently || err == io.EOF {
			...
			util.LOG_TRACE("WEBDAV_MKDIR", "Can't create directory %s: %v; error ignored", dir, err)
			storage.directoryCacheLock.Lock()
...
func (storage *WebDAVStorage) DownloadFile(threadIndex int, filePath string, chunk io.Writer) (err error) {
