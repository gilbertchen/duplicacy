...
func (storage *ACDStorage) DeleteFile(threadIndex int, filePath string) (err error) {
	...
	if fileID == "" {
		util.LOG_TRACE("ACD_STORAGE", "File '%s' to be deleted does not exist", filePath)
		return nil
	}

	err = storage.client.DeleteFile(fileID)
	if e, ok := err.(ACDError); ok && e.Status == 409 {
		util.LOG_DEBUG("ACD_DELETE", "Ignore 409 conflict error")
		return nil
	}
	return err
}
...
func (storage *ACDStorage) MoveFile(threadIndex int, from string, to string) (err error) {
	...
	if err != nil {
		if e, ok := err.(ACDError); ok && e.Status == 409 {
			util.LOG_DEBUG("ACD_MOVE", "Ignore 409 conflict error")
		} else {
			return err
		}
	}
...
func (storage *ACDStorage) DownloadFile(threadIndex int, filePath string, chunk io.Writer) (err error) {
	fileID, err := storage.getIDFromPath(threadIndex, filePath, false)
	if err != nil {
		return err
	}
	...
	_, err = util.RateLimitedCopy(chunk, readCloser, storage.DownloadRateLimit/storage.numberOfThreads)
	return err
}
...
func (storage *ACDStorage) UploadFile(threadIndex int, filePath string, content []byte) (err error) {
	...
	if e, ok := err.(ACDError); ok && e.Status == 409 {
		util.LOG_TRACE("ACD_UPLOAD", "File %s already exists", filePath)
		return nil
	} else {
		return err
	}
