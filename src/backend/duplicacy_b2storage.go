...
func (storage *B2Storage) MoveFile(threadIndex int, from string, to string) (err error) {
	...
	if filePath == "" {
		util.LOG_FATAL("STORAGE_MOVE", "Moving file '%s' to '%s' is not supported", from, to)
		return nil
	}
...
func (storage *B2Storage) DownloadFile(threadIndex int, filePath string, chunk io.Writer) (err error) {
	...
	_, err = util.RateLimitedCopy(chunk, readCloser, storage.DownloadRateLimit/storage.client.Threads)
	return err
}
