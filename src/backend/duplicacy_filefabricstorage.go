...
func (storage *FileFabricStorage) shouldRetry(retry int, messageFormat string, messageArguments ...interface{}) bool {
	...
	if retry >= storage.maxRetries {
		util.LOG_WARN("FILEFABRIC_REQUEST", "%s", message)
		return false
	}
	...
	util.LOG_INFO("FILEFABRIC_RETRY", "%s; retrying after %.1f seconds", message, float32(delay)/1000.0)
...
func (storage *FileFabricStorage) sendRequest(threadIndex int, method string, requestURL string, requestHeaders map[string]string, input interface{}) (io.ReadCloser, http.Header, int64, error) {
	...
	for retries := 0; ; retries++ {
		...
		default:
			util.LOG_FATAL("FILEFABRIC_REQUEST", "Input type is not supported")
		...
		if _, ok := input.(*util.RateLimitedReader); ok {
			request.ContentLength = input.(*util.RateLimitedReader).Length()
		}
...
func (storage *FileFabricStorage) DownloadFile(threadIndex int, filePath string, chunk io.Writer) (err error) {
	...
	defer readCloser.Close()
	defer io.Copy(ioutil.Discard, readCloser)
	_, err = util.RateLimitedCopy(chunk, readCloser, storage.DownloadRateLimit/storage.threads)
	return err
}
...
func (storage *FileFabricStorage) UploadFile(threadIndex int, filePath string, content []byte) (err error) {
	...
	rateLimitedReader := util.CreateRateLimitedReader(requestBody.Bytes(), storage.UploadRateLimit/storage.threads)
