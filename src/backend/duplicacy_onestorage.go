...
func (storage *OneDriveStorage) DeleteFile(threadIndex int, filePath string) (err error) {
	...
	if e, ok := err.(OneDriveError); ok && e.Status == 404 {
		util.LOG_DEBUG("ONEDRIVE_DELETE", "Ignore 404 error")
		return nil
	}
	return err
}
...
func (storage *OneDriveStorage) MoveFile(threadIndex int, from string, to string) (err error) {
	...
	if err != nil {
		if e, ok := err.(OneDriveError); ok && e.Status == 409 {
			util.LOG_DEBUG("ONEDRIVE_MOVE", "Ignore 409 conflict error")
		} else {
			return err
		}
	}
...
func (storage *OneDriveStorage) DownloadFile(threadIndex int, filePath string, chunk io.Writer) (err error) {
	...
	_, err = util.RateLimitedCopy(chunk, readCloser, storage.DownloadRateLimit/storage.numberOfThread)
	return err
}
...
func (storage *OneDriveStorage) UploadFile(threadIndex int, filePath string, content []byte) (err error) {
	...
	if e, ok := err.(OneDriveError); ok && e.Status == 409 {
		util.LOG_TRACE("ONEDRIVE_UPLOAD", "File %s already exists", filePath)
		return nil
	} else {
		return err
	}
