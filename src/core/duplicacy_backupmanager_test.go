...
func createRandomFile(path string, maxSize int) {
	...
	if err != nil {
		util.LOG_ERROR("RANDOM_FILE", "Can't open %s for writing: %v", path, err)
		return
	}
	...
	for size > 0 {
		...
		if err != nil {
			util.LOG_ERROR("RANDOM_FILE", "Failed to write to %s: %v", path, err)
			return
		}
		size -= bytes
	}
...
func modifyFile(path string, portion float32) {
	...
	if err != nil {
		util.LOG_ERROR("MODIFY_FILE", "Can't stat the file %s: %v", path, err)
		return
	}
	...
	if err != nil {
		util.LOG_ERROR("MODIFY_FILE", "Can't open %s for writing: %v", path, err)
		return
	}
	...
	if err != nil {
		util.LOG_ERROR("MODIFY_FILE", "Can't seek to the end of the file %s: %v", path, err)
		return
	}
	...
	if err != nil {
		util.LOG_ERROR("MODIFY_FILE", "Can't seek to the offset %d: %v", start, err)
		return
	}
	...
	if err != nil {
		util.LOG_ERROR("MODIFY_FILE", "Failed to write to %s: %v", path, err)
		return
	}
	...
	if err != nil {
		util.LOG_ERROR("MODIFY_FILE", "Failed to change the modification time of %s: %v", path, err)
		return
	}
}
...
func truncateFile(path string) {
	file, err := os.OpenFile(path, os.O_WRONLY, 0644)
	if err != nil {
		util.LOG_ERROR("TRUNCATE_FILE", "Can't open %s for writing: %v", path, err)
		return
	}
	...
	if err != nil {
		util.LOG_ERROR("TRUNCATE_FILE", "Can't seek to the end of the file %s: %v", path, err)
		return
	}
	...
	if err != nil {
		util.LOG_ERROR("TRUNCATE_FILE", "Can't truncate the file %s to size %d: %v", path, newSize, err)
		return
	}
}
...
func getFileHash(path string) (hash string) {
	...
	if err != nil {
		util.LOG_ERROR("FILE_HASH", "Can't open %s for reading: %v", path, err)
		return ""
	}
	...
	if err != nil {
		util.LOG_ERROR("FILE_HASH", "Can't read file %s: %v", path, err)
		return ""
	}
...
func TestBackupManager(t *testing.T) {
	...
	delay := 0
	if _, ok := storage.(*backend.ACDStorage); ok {
		delay = 1
	}
	if _, ok := storage.(*backend.OneDriveStorage); ok {
		delay = 5
	}
	...
	util.SetDuplicacyPreferencePath(testDir + "/repository1/.duplicacy")
	...
	time.Sleep(time.Duration(delay) * time.Second)
	util.SetDuplicacyPreferencePath(testDir + "/repository2/.duplicacy")
...
func corruptFile(path string, start int, length int, seed int64) {
	...
	file, err := os.OpenFile(path, os.O_WRONLY, 0644)
	if err != nil {
		util.LOG_ERROR("CORRUPT_FILE", "Can't open %s for writing: %v", path, err)
		return
	}
	...
	_, err = file.Seek(int64(start), 0)
	if err != nil {
		util.LOG_ERROR("CORRUPT_FILE", "Can't seek to the offset %d: %v", start, err)
		return
	}
	...
	_, err = file.Write(buffer)
	if err != nil {
		util.LOG_ERROR("CORRUPT_FILE", "Failed to write to %s: %v", path, err)
		return
	}
}
...
func TestPersistRestore(t *testing.T) {
	...
	delay := 0
	if _, ok := unencStorage.(*backend.ACDStorage); ok {
		delay = 1
	}
	if _, ok := unencStorage.(*backend.OneDriveStorage); ok {
		delay = 5
	}
	...
	util.SetDuplicacyPreferencePath(testDir + "/repository3/.duplicacy")
