	...
	for i := file.StartChunk; i <= file.EndChunk; i++ {
		...
		if task.needed {
			if !task.isDownloading {
				...
				util.LOG_DEBUG("DOWNLOAD_PREFETCH", "Prefetching %s chunk %s", file.Path,
			...
			util.LOG_DEBUG("DOWNLOAD_PREFETCH", "%s chunk %s is not needed", file.Path,
				downloader.operator.config.GetChunkIDFromHash(task.chunkHash))
		}
	}
}
...
func (downloader *ChunkDownloader) WaitForChunk(chunkIndex int) (chunk *Chunk) {
	...
	if !downloader.taskList[chunkIndex].isDownloading {
		util.LOG_DEBUG("DOWNLOAD_FETCH", "Fetching chunk %s",
	...
	for i := chunkIndex + 1; i < len(downloader.taskList); i++ {
		...
		if !task.isDownloading {
			util.LOG_DEBUG("DOWNLOAD_PREFETCH", "Prefetching chunk %s", downloader.operator.config.GetChunkIDFromHash(task.chunkHash))
