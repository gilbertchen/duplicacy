...
func ReleaseChunkBuffer(buffer *bytes.Buffer) {
	...
	default:
		util.LOG_INFO("CHUNK_BUFFER", "Discarding a free chunk buffer due to a full pool")
	}
}
...
func (chunk *Chunk) VerifyID() {
	...
	if chunkID != chunk.GetID() {
		util.LOG_ERROR("CHUNK_ID", "The chunk id should be %s instead of %s, length: %d", chunkID, chunk.GetID(), len(chunk.buffer.Bytes()))
	}
}
...
func (chunk *Chunk) Decrypt(encryptionKey []byte, derivationKey string) (err error, rewriteNeeded bool) {
	...
	if len(encryptedBuffer.Bytes()) > bannerLength && string(encryptedBuffer.Bytes()[:bannerLength]) == ERASURE_CODING_BANNER {
		...
		util.LOG_DEBUG("CHUNK_ERASURECODE", "Chunk size: %d bytes, data size: %d, parity: %d/%d", chunkSize, len(encryptedBuffer.Bytes()), dataShards, parityShards)
		if len(encryptedBuffer.Bytes()) > expectedLength {
			util.LOG_WARN("CHUNK_ERASURECODE", "Chunk has %d bytes (instead of %d)", len(encryptedBuffer.Bytes()), expectedLength)
		} else if len(encryptedBuffer.Bytes()) == expectedLength {
			// Correct size; fall through
		} else if len(encryptedBuffer.Bytes()) > minimumLength {
			util.LOG_WARN("CHUNK_ERASURECODE", "Chunk is truncated (%d out of %d bytes)", len(encryptedBuffer.Bytes()), expectedLength)
		} else {
				...
				if err == nil {
					...
					if err == nil {
						...
						if matched && !wrongHashDetected {
							util.LOG_WARN("CHUNK_ERASURECODE", "Hash for shard %d was calculated with a wrong version of highwayhash", i)
		...
		if !recoveryNeeded {
			...
			util.LOG_WARN("CHUNK_ERASURECODE", "Recovering a %d byte chunk from %d byte shards: %s", chunkSize, shardSize, slots)
			...
			util.LOG_DEBUG("CHUNK_ERASURECODE", "Chunk data successfully recovered")
			buffer := AllocateChunkBuffer()
			buffer.Reset()
	...
	if len(encryptionKey) > 0 {
		...
		if encryptionVersion == ENCRYPTION_VERSION_RSA {
			if chunk.config.rsaPrivateKey == nil {
				util.LOG_ERROR("CHUNK_DECRYPT", "An RSA private key is required to decrypt the chunk")
