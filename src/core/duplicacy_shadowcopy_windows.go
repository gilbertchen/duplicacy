...
func (async *IVSSAsync) Wait(seconds int) bool {
	...
	for {
		...
		if ret != 0 {
			util.LOG_WARN("IVSSASYNC_WAIT", "IVssAsync::Wait returned %d\n", ret)
		}
		...
		if ret != 0 {
			util.LOG_WARN("IVSSASYNC_QUERY", "IVssAsync::QueryStatus returned %d\n", ret)
		}
		...
		if time.Now().Unix()-startTime > int64(seconds) {
			util.LOG_WARN("IVSSASYNC_TIMEOUT", "IVssAsync is pending for more than %d seconds\n", seconds)
			return false
		}
	}
}
...
func getIVSSAsync(unknown *ole.IUnknown, iid *ole.GUID) (async *IVSSAsync) {
	...
	if r != 0 {
		util.LOG_WARN("IVSSASYNC_QUERY", "IVSSAsync::QueryInterface returned %d\n", r)
		return nil
	}
	return
}
...
func getIVSS(unknown *ole.IUnknown, iid *ole.GUID) (ivss *IVSS) {
	...
	if r != 0 {
		util.LOG_WARN("IVSS_QUERY", "IVSS::QueryInterface returned %d\n", r)
		return nil
	}
...
func DeleteShadowCopy() {
	if vssBackupComponent != nil {
		...
		util.LOG_TRACE("VSS_DELETE", "Deleting the shadow copy used for this backup")
		ret, _, _ := vssBackupComponent.DeleteSnapshots(snapshotID)
		if ret != 0 {
			util.LOG_WARN("VSS_DELETE", "Failed to delete the shadow copy: %x\n", uint(ret))
		} else {
			util.LOG_INFO("VSS_DELETE", "The shadow copy has been successfully deleted")
		}
	}
	...
	if shadowLink != "" {
		...
		if err != nil {
			util.LOG_WARN("VSS_SYMLINK", "Failed to remove the symbolic link for the shadow copy: %v", err)
		}
	}
...
func CreateShadowCopy(top string, shadowCopy bool, timeoutInSeconds int) (shadowTop string) {
	...
	if len(top) < 3 || top[1] != ':' || (top[2] != '/' && top[2] != '\\') {
		util.LOG_ERROR("VSS_PATH", "Invalid repository path: %s", top)
		return top
	}
	volume := top[:1] + ":\\"

	util.LOG_INFO("VSS_CREATE", "Creating a shadow copy for %s", volume)

	var unknown *ole.IUnknown
	r, _, err := procCreateVssBackupComponents.Call(uintptr(unsafe.Pointer(&unknown)))

	if r == 0x80070005 {
		util.LOG_ERROR("VSS_CREATE", "Only administrators can create shadow copies")
		return top
	}

	if r != 0 {
		util.LOG_ERROR("VSS_CREATE", "Failed to create the VSS backup component: %d", r)
		return top
	}

	vssBackupComponent = getIVSS(unknown, IID_IVSS)
	if vssBackupComponent == nil {
		util.LOG_ERROR("VSS_CREATE", "Failed to create the VSS backup component")
		return top
	}

	ret := vssBackupComponent.InitializeForBackup()
	if ret != 0 {
		util.LOG_ERROR("VSS_INIT", "Shadow copy creation failed: InitializeForBackup returned %x", uint(ret))
		return top
	}
	...
	if ret != 0 {
		util.LOG_ERROR("VSS_GATHER", "Shadow copy creation failed: GatherWriterMetadata returned %x", uint(ret))
		return top
	}

	if async == nil {
		util.LOG_ERROR("VSS_GATHER",
			"Shadow copy creation failed: GatherWriterMetadata failed to return a valid IVssAsync object")
		return top
	}

	if !async.Wait(timeoutInSeconds) {
		util.LOG_ERROR("VSS_GATHER", "Shadow copy creation failed: GatherWriterMetadata didn't finish properly")
		return top
	}
	async.Release()
	...
	if ret != 0 {
		util.LOG_ERROR("VSS_START", "Shadow copy creation failed: StartSnapshotSet returned %x", uint(ret))
		return top
	}

	ret = vssBackupComponent.AddToSnapshotSet(volume, &snapshotID)
	if ret != 0 {
		util.LOG_ERROR("VSS_ADD", "Shadow copy creation failed: AddToSnapshotSet returned %x", uint(ret))
		return top
	}

	s, _ := ole.StringFromIID(&snapshotID)
	util.LOG_DEBUG("VSS_ID", "Creating shadow copy %s", s)

	ret = vssBackupComponent.SetBackupState()
	if ret != 0 {
		util.LOG_ERROR("VSS_SET", "Shadow copy creation failed: SetBackupState returned %x", uint(ret))
		return top
	}

	ret, async = vssBackupComponent.PrepareForBackup()
	if ret != 0 {
		util.LOG_ERROR("VSS_PREPARE", "Shadow copy creation failed: PrepareForBackup returned %x", uint(ret))
		return top
	}
	if async == nil {
		util.LOG_ERROR("VSS_PREPARE",
			"Shadow copy creation failed: PrepareForBackup failed to return a valid IVssAsync object")
		return top
	}

	if !async.Wait(timeoutInSeconds) {
		util.LOG_ERROR("VSS_PREPARE", "Shadow copy creation failed: PrepareForBackup didn't finish properly")
		return top
	}
	async.Release()
	...
	if ret != 0 {
		util.LOG_ERROR("VSS_SNAPSHOT", "Shadow copy creation failed: DoSnapshotSet returned %x", uint(ret))
		return top
	}
	if async == nil {
		util.LOG_ERROR("VSS_SNAPSHOT",
			"Shadow copy creation failed: DoSnapshotSet failed to return a valid IVssAsync object")
		return top
	}

	if !async.Wait(timeoutInSeconds) {
		util.LOG_ERROR("VSS_SNAPSHOT", "Shadow copy creation failed: DoSnapshotSet didn't finish properly")
		return top
	}
	async.Release()
	...
	if ret != 0 {
		util.LOG_ERROR("VSS_PROPERTIES", "GetSnapshotProperties returned %x", ret)
		return top
	}
	...
	util.LOG_DEBUG("VSS_PROPERTY", "SnapshotID: %s", SnapshotIDString)
	util.LOG_DEBUG("VSS_PROPERTY", "SnapshotSetID: %s", SnapshotSetIDString)

	util.LOG_DEBUG("VSS_PROPERTY", "SnapshotDeviceObject: %s", uint16ArrayToString(properties.SnapshotDeviceObject))
	util.LOG_DEBUG("VSS_PROPERTY", "OriginalVolumeName: %s", uint16ArrayToString(properties.OriginalVolumeName))
	util.LOG_DEBUG("VSS_PROPERTY", "OriginatingMachine: %s", uint16ArrayToString(properties.OriginatingMachine))
	util.LOG_DEBUG("VSS_PROPERTY", "OriginatingMachine: %s", uint16ArrayToString(properties.OriginatingMachine))
	util.LOG_DEBUG("VSS_PROPERTY", "ServiceMachine: %s", uint16ArrayToString(properties.ServiceMachine))
	util.LOG_DEBUG("VSS_PROPERTY", "ExposedName: %s", uint16ArrayToString(properties.ExposedName))
	util.LOG_DEBUG("VSS_PROPERTY", "ExposedPath: %s", uint16ArrayToString(properties.ExposedPath))

	util.LOG_INFO("VSS_DONE", "Shadow copy %s created", SnapshotIDString)
	...
	if err != nil {
		util.LOG_ERROR("VSS_SYMLINK", "Failed to create a symbolic link to the shadow copy just created: %v", err)
		return top
	}
