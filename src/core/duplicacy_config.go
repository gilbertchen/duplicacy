...
func (config *Config) Print() {
	...
	util.LOG_INFO("CONFIG_INFO", "Compression level: %d", config.CompressionLevel)
	util.LOG_INFO("CONFIG_INFO", "Average chunk size: %d", config.AverageChunkSize)
	util.LOG_INFO("CONFIG_INFO", "Maximum chunk size: %d", config.MaximumChunkSize)
	util.LOG_INFO("CONFIG_INFO", "Minimum chunk size: %d", config.MinimumChunkSize)
	util.LOG_INFO("CONFIG_INFO", "Chunk seed: %x", config.ChunkSeed)

	util.LOG_TRACE("CONFIG_INFO", "Hash key: %x", config.HashKey)
	util.LOG_TRACE("CONFIG_INFO", "ID key: %x", config.IDKey)

	if len(config.ChunkKey) > 0 {
		util.LOG_TRACE("CONFIG_INFO", "File chunks are encrypted")
	}

	if len(config.FileKey) > 0 {
		util.LOG_TRACE("CONFIG_INFO", "Metadata chunks are encrypted")
	}

	if config.DataShards != 0 && config.ParityShards != 0 {
		util.LOG_TRACE("CONFIG_INFO", "Data shards: %d, parity shards: %d", config.DataShards, config.ParityShards)
	}
	...
	if config.rsaPublicKey != nil {
		...
		util.LOG_TRACE("CONFIG_INFO", "RSA public key: %s", publicKey)
	}
...
func (config *Config) PrintCompressionLevel() {
	for name, level := range ZSTD_COMPRESSION_LEVELS {
		if level == config.CompressionLevel {
			util.LOG_INFO("COMPRESSION_LEVEL", "Zstd compression is enabled (level: %s)", name)
		}
	}
}

var _ backend.ConfigI = (*Config)(nil)

func (config *Config) IsFixedNesting() bool {
	return config.FixedNesting
}

func (config *Config) NestingFile(nesting string, DerivedStorage backend.Storage) ([]byte, error) {
	nestingFile := CreateChunk(CreateConfig(), true)
	err := DerivedStorage.DownloadFile(0, nesting, nestingFile)
	if err != nil {
		return nil, err
	}
	return nestingFile.GetBytes(), nil
}
...
func CreateConfigFromParameters(compressionLevel int, averageChunkSize int, maximumChunkSize int, mininumChunkSize int,
	...
	if isEncrypted {
		...
		if err != nil {
			util.LOG_ERROR("CONFIG_KEY", "Failed to generate random keys: %v", err)
			return nil
		}
...
func (config *Config) GetChunk() (chunk *Chunk) {
	...
	default:
		...
		if numberOfChunks >= int32(runtime.NumCPU()*16) {
			util.LOG_WARN("CONFIG_CHUNK", "%d chunks have been allocated", numberOfChunks)
...
func (config *Config) PutChunk(chunk *Chunk) {
	...
	default:
		util.LOG_INFO("CHUNK_BUFFER", "Discarding a free chunk due to a full pool")
	}
}
...
func (config *Config) NewKeyedHasher(key []byte) hash.Hash {
	...
	if config.CompressionLevel >= DEFAULT_COMPRESSION_LEVEL {
		...
		if err != nil {
			util.LOG_ERROR("HASH_KEY", "Invalid hash key: %x", key)
		}
		return hasher
	} else {
...
func DownloadConfig(storage backend.Storage, password string) (config *Config, isEncrypted bool, err error) {
	...
	if len(password) > 0 {
		...
		if string(configFile.GetBytes()[:len(ENCRYPTION_BANNER)]) == ENCRYPTION_BANNER {
			...
			masterKey = util.GenerateKeyFromPassword(password, DEFAULT_KEY, CONFIG_DEFAULT_ITERATIONS)
			util.LOG_TRACE("CONFIG_FORMAT", "Using a static salt and %d iterations for key derivation", CONFIG_DEFAULT_ITERATIONS)
			...
			util.LOG_TRACE("CONFIG_ITERATIONS", "Using %d iterations for key derivation", iterations)
			masterKey = util.GenerateKeyFromPassword(password, saltStart[:CONFIG_SALT_LENGTH], int(iterations))
			...
			if len(configFile.GetBytes()) != encryptedLength {
				util.LOG_ERROR("CONFIG_DOWNLOAD", "Encrypted config has %d bytes instead of expected %d bytes", len(configFile.GetBytes()), encryptedLength)
			}
		} else {
...
func UploadConfig(storage backend.Storage, config *Config, password string, iterations int) bool {
	...
	if len(password) > 0 {
		...
		if len(password) < 8 {
			util.LOG_ERROR("CONFIG_PASSWORD", "The password must be at least 8 characters")
			return false
		}

		_, err := rand.Read(salt)
		if err != nil {
			util.LOG_ERROR("CONFIG_KEY", "Failed to generate random salt: %v", err)
			return false
		}

		masterKey = util.GenerateKeyFromPassword(password, salt, iterations)
	}
	...
	if len(password) > 0 {
		...
		if err != nil {
			util.LOG_ERROR("CONFIG_CREATE", "Failed to create the config file: %v", err)
			return false
		}
		...
		if len(chunk.GetBytes()) != encryptedLength {
			util.LOG_ERROR("CONFIG_CREATE", "Encrypted config has %d bytes instead of expected %d bytes", len(chunk.GetBytes()), encryptedLength)
		}
	}

	err = storage.UploadFile(0, "config", chunk.GetBytes())
	if err != nil {
		util.LOG_ERROR("CONFIG_INIT", "Failed to configure the storage: %v", err)
		return false
	}
	...
	for _, subDir := range []string{"chunks", "snapshots"} {
		...
		if err != nil {
			util.LOG_ERROR("CONFIG_MKDIR", "Failed to create storage subdirectory: %v", err)
		}
	}
...
// is enabled.
func ConfigStorage(storage backend.Storage, iterations int, compressionLevel int, averageChunkSize int, maximumChunkSize int,
	minimumChunkSize int, password string, copyFrom *Config, bitCopy bool, keyFile string, dataShards int, parityShards int) bool {

	exist, _, _, err := storage.GetFileInfo(0, "config")
	if err != nil {
		util.LOG_ERROR("CONFIG_INIT", "Failed to check if there is an existing config file: %v", err)
		return false
	}

	if exist {
		util.LOG_INFO("CONFIG_EXIST", "The storage has already been configured")
		return false
	}
...
func (config *Config) loadRSAPublicKey(keyFile string) {
	...
	if !strings.Contains(keyFile, "-----BEGIN") {
		...
		if err != nil {
			util.LOG_ERROR("BACKUP_KEY", "Failed to read the public key file: %v", err)
			return
		}
	}
	...
	if decodedKey == nil {
		util.LOG_ERROR("RSA_PUBLIC", "unrecognized public key in %s", keyFile)
		return
	}
	if decodedKey.Type != "PUBLIC KEY" {
		util.LOG_ERROR("RSA_PUBLIC", "Unsupported public key type %s in %s", decodedKey.Type, keyFile)
		return
	}

	parsedKey, err := x509.ParsePKIXPublicKey(decodedKey.Bytes)
	if err != nil {
		util.LOG_ERROR("RSA_PUBLIC", "Failed to parse the public key in %s: %v", keyFile, err)
		return
	}

	key, ok := parsedKey.(*rsa.PublicKey)
	if !ok {
		util.LOG_ERROR("RSA_PUBLIC", "Unsupported public key type %s in %s", reflect.TypeOf(parsedKey), keyFile)
		return
	}
...
func (config *Config) loadRSAPrivateKey(keyFile string, passphrase string) {
	...
	if config.rsaPublicKey == nil {
		util.LOG_ERROR("RSA_PUBLIC", "The storage was not encrypted by an RSA key")
		return
	}
	...
	if !strings.Contains(keyFile, "-----BEGIN") {
		encodedKey, err = ioutil.ReadFile(keyFile)
		if err != nil {
			util.LOG_ERROR("RSA_PRIVATE", "Failed to read the private key file: %v", err)
			return
		}
	}
	...
	decodedKey, _ := pem.Decode(encodedKey)
	if decodedKey == nil {
		util.LOG_ERROR("RSA_PRIVATE", "unrecognized private key in %s", keyFile)
		return
	}
	if decodedKey.Type != "RSA PRIVATE KEY" {
		util.LOG_ERROR("RSA_PRIVATE", "Unsupported private key type %s in %s", decodedKey.Type, keyFile)
		return
	}
	...
	if parsedKey, err = x509.ParsePKCS1PrivateKey(decodedKeyBytes); err != nil {
		if parsedKey, err = x509.ParsePKCS8PrivateKey(decodedKeyBytes); err != nil {
			util.LOG_ERROR("RSA_PRIVATE", "Failed to parse the private key in %s: %v", keyFile, err)
			return
		}
	}
	...
	if !ok {
		util.LOG_ERROR("RSA_PRIVATE", "Unsupported private key type %s in %s", reflect.TypeOf(parsedKey), keyFile)
		return
	}
	...
	if err != nil {
		util.LOG_ERROR("RSA_PRIVATE", "Failed to generate random data for testing the private key: %v", err)
		return
	}
	...
	if err != nil {
		util.LOG_ERROR("RSA_PRIVATE", "Failed to encrypt random data with the public key: %v", err)
		return
	}

	decryptedData, err := rsa.DecryptOAEP(sha256.New(), rand.Reader, key, encryptedData, nil)
	if err != nil {
		util.LOG_ERROR("RSA_PRIVATE", "Incorrect private key: %v", err)
		return
	}

	if !bytes.Equal(data, decryptedData) {
		util.LOG_ERROR("RSA_PRIVATE", "Decrypted data do not match the original data")
		return
	}
