...
func createTestSnapshotManager(testDir string) *SnapshotManager {
	...
	storage, _ := backend.CreateFileStorage(testDir, false, 1)
	...
	snapshotCache, _ := backend.CreateFileStorage(cacheDir, false, 1)
	...
	util.SetDuplicacyPreferencePath(testDir + "/.duplicacy")
...
func uploadTestChunk(manager *SnapshotManager, content []byte) string {
	...
	chunkOperator.UploadCompletionFunc = func(chunk *Chunk, chunkIndex int, skipped bool, chunkSize int, uploadSize int) {
		util.LOG_INFO("UPLOAD_CHUNK", "Chunk %s size %d uploaded", chunk.GetID(), chunkSize)
	}
...
func uploadRandomChunk(manager *SnapshotManager, chunkSize int) string {
	...
	if err != nil {
		util.LOG_ERROR("UPLOAD_RANDOM", "Error generating random content: %v", err)
		return ""
	}
...
func checkTestSnapshots(manager *SnapshotManager, expectedSnapshots int, expectedFossils int) {
	...
	for chunk, referenced := range chunks {
		if !referenced {
			util.LOG_INFO("UNREFERENCED_CHUNK", "Unreferenced chunk %s", chunk)
			numberOfFossils++
		}
	}

	if numberOfSnapshots != expectedSnapshots {
		util.LOG_ERROR("SNAPSHOT_COUNT", "Expecting %d snapshots, got %d instead", expectedSnapshots, numberOfSnapshots)
	}

	if numberOfFossils != expectedFossils {
		util.LOG_ERROR("FOSSIL_COUNT", "Expecting %d unreferenced chunks, got %d instead", expectedFossils, numberOfFossils)
	}
}
