...
func CreateFileChunkMaker(config *Config, hashOnly bool) *ChunkMaker {
	...
	if size != config.AverageChunkSize {
		util.LOG_FATAL("CHUNK_SIZE", "Invalid average chunk size: %d is not a power of 2", config.AverageChunkSize)
		return nil
	}
...
func CreateMetaDataChunkMaker(config *Config, chunkSize int) *ChunkMaker {
	...
	if size != chunkSize {
		util.LOG_FATAL("CHUNK_SIZE", "Invalid metadata chunk size: %d is not a power of 2", chunkSize)
		return nil
	}
	...
	if maker.minimumChunkSize == maker.maximumChunkSize {
		...
		for {
			...
			for maker.bufferStart < maker.minimumChunkSize && !isEOF {
				...
				if err != nil {
					if err != io.EOF {
						util.LOG_ERROR("CHUNK_MAKER", "Failed to read %d bytes: %s", count, err.Error())
						return 0, ""
					} else {
						isEOF = true
					}
