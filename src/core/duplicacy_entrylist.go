...
func (entryList *EntryList) SaveIncompleteSnapshot() {
	...
	if entryList.onDiskFile == nil {
		err := entryList.createOnDiskFile()
		if err != nil {
			util.LOG_WARN("INCOMPLETE_SAVE", "Failed to create the incomplete snapshot file: %v", err)
			return
		}
		...
		for _, entry := range entryList.entries {
			...
			err = entry.EncodeWithHash(entryList.encoder)
			if err != nil {
				util.LOG_WARN("INCOMPLETE_SAVE", "Failed to save the entry %s: %v", entry.Path, err)
				return
			}
		}
	}
	...
	err := entryList.onDiskFile.Close()
	if err != nil {
		util.LOG_WARN("INCOMPLETE_SAVE", "Failed to close the on-disk file: %v", err)
		return
	}
	...
	if _, err := os.Stat(filePath); err == nil {
		...
		if err != nil {
			util.LOG_WARN("INCOMPLETE_REMOVE", "Failed to remove previous incomplete snapshot: %v", err)
		}
	}

	err = os.Rename(path.Join(entryList.cachePath, "incomplete_files"), filePath)
	if err != nil {
		util.LOG_WARN("INCOMPLETE_SAVE", "Failed to rename the incomplete snapshot file: %v", err)
		return
	}
	...
	if err != nil {
		util.LOG_WARN("INCOMPLETE_SAVE", "Failed to create the incomplete chunk file: %v", err)
		return
	}
	...
	if err != nil {
		util.LOG_WARN("INCOMPLETE_SAVE", "Failed to save the incomplete snapshot: %v", err)
		return
	}

	util.LOG_INFO("INCOMPLETE_SAVE", "Incomplete snapshot saved to %s", filePath)
}
...
func loadIncompleteSnapshot(snapshotID string, cachePath string) *EntryList {
	...
	if err != nil {
		util.LOG_WARN("INCOMPLETE_LOAD", "Failed to open the incomplete snapshot: %v", err)
		return nil
	}
	...
	if err != nil {
		util.LOG_WARN("INCOMPLETE_LOAD", "Failed to load the incomplete snapshot: %v", err)
		return nil
	}

	checksum := entryList.CalculateChecksum()
	if checksum != entryList.Checksum {
		util.LOG_WARN("INCOMPLETE_LOAD", "Failed to load the incomplete snapshot: checksum mismatched")
		return nil
	}

	onDiskFile, err := os.OpenFile(onDiskFilePath, os.O_RDONLY, 0600)
	if err != nil {
		util.LOG_WARN("INCOMPLETE_LOAD", "Failed to open the on disk file for the incomplete snapshot: %v", err)
		return nil
	}
	...
	if err != nil {
		util.LOG_WARN("INCOMPLETE_LOAD", "Failed to read the token for the incomplete snapshot: %v", err)
		onDiskFile.Close()
		return nil
	}

	if token != entryList.Token {
		util.LOG_WARN("INCOMPLETE_LOAD", "Mismatched tokens in the incomplete snapshot")
		onDiskFile.Close()
		return nil
	}
	...
	util.LOG_INFO("INCOMPLETE_LOAD", "Previous incomplete backup contains %d files and %d chunks",
...
func deleteIncompleteSnapshot(cachePath string) {
	...
	for _, file := range []string{"incomplete_snapshot", "incomplete_chunks", "incomplete_files"} {
		...
		if _, err := os.Stat(filePath); err == nil {
			err = os.Remove(filePath)
			if err != nil {
				util.LOG_WARN("INCOMPLETE_REMOVE", "Failed to remove the incomplete snapshot: %v", err)
				return
			}
		}
	}
