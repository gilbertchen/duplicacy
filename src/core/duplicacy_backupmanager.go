...
type BackupManager struct {
	...
	storage    backend.Storage // the storage for storing backups

	SnapshotManager *SnapshotManager     // the snapshot manager
	snapshotCache   *backend.FileStorage // for copies of chunks needed by snapshots
...
func CreateBackupManager(snapshotID string, storage backend.Storage, top string, password string, nobackupFile string, filtersFile string, excludeByAttribute bool) *BackupManager {
...
func (manager *BackupManager) SetupSnapshotCache(storageName string) bool {
	...
	storage, err := backend.CreateFileStorage(cacheDir, false, 1)
	if err != nil {
		util.LOG_ERROR("BACKUP_CACHE", "Failed to create the snapshot cache dir: %v", err)
		return false
	}
	...
	for _, subdir := range []string{"chunks", "snapshots"} {
		...
		if err != nil && !os.IsExist(err) {
			util.LOG_ERROR("BACKUP_CACHE", "Failed to create the snapshot cache subdir: %v", err)
			return false
		}
	}
...
func (manager *BackupManager) Backup(top string, quickMode bool, threads int, tag string,
	...
	if err != nil {
		util.LOG_ERROR("REPOSITORY_ERR", "Failed to obtain the absolute path of the repository: %v", err)
		return false
	}

	startTime := time.Now().Unix()

	util.LOG_DEBUG("BACKUP_PARAMETERS", "top: %s, quick: %t, tag: %s", top, quickMode, tag)

	manager.config.PrintCompressionLevel()

	if manager.config.DataShards != 0 && manager.config.ParityShards != 0 {
		util.LOG_INFO("BACKUP_ERASURECODING", "Erasure coding is enabled with %d data shards and %d parity shards",
			manager.config.DataShards, manager.config.ParityShards)
	}

	if manager.config.rsaPublicKey != nil && len(manager.config.FileKey) > 0 {
		util.LOG_INFO("BACKUP_KEY", "RSA encryption is enabled")
	}

	if manager.excludeByAttribute {
		util.LOG_INFO("BACKUP_EXCLUDE", "Exclude files with no-backup attributes")
	}

	remoteSnapshot := manager.SnapshotManager.downloadLatestSnapshot(manager.snapshotID)
	if remoteSnapshot == nil {
		util.LOG_INFO("BACKUP_START", "No previous backup found")
		remoteSnapshot = CreateEmptySnapshot(manager.snapshotID)
	} else {
		util.LOG_INFO("BACKUP_START", "Last backup at revision %d found", remoteSnapshot.Revision)
	}
	...
	if manager.storage.IsFastListing() && remoteSnapshot.Revision == 0 {
		util.LOG_INFO("BACKUP_LIST", "Listing all chunks")
		...
		if incompleteSnapshot != nil && !incompleteSnapshot.CheckChunks(manager.config, chunkCache) {
			util.LOG_WARN("INCOMPLETE_DISCARD", "The incomplete snapshot can't be used as it contains chunks not in the storage")
	...
	util.LOG_INFO("BACKUP_INDEXING", "Indexing %s", top)
	go func() {
		// List local files
	...
	if err != nil {
		util.LOG_ERROR("BACKUP_CREATE", "Failed to create the entry list: %v", err)
		return false
	}
	...
	if localEntryList.NumberOfEntries == 0 {
		util.LOG_ERROR("SNAPSHOT_EMPTY", "No files under the repository to be backed up")
		return false
	}
	...
	if os.Getenv("DUPLICACY_UPLOAD_KEEPALIVE") != "" {
		...
		util.LOG_INFO("UPLOAD_KEEPALIVE", "Setting KeepUploadAlive to %d second", value)
	...
	if value, found := os.LookupEnv("DUPLICACY_FAIL_CHUNK"); found {
		...
		util.LOG_INFO("SNAPSHOT_FAIL", "Will abort the backup on chunk %d", chunkToFail)
	}
	...
	uploadChunkCompletionFunc := func(chunk *Chunk, chunkIndex int, inCache bool, chunkSize int, uploadSize int) {
		...
		action := "Skipped"
		if inCache {
			util.LOG_DEBUG("CHUNK_CACHE", "Skipped chunk %s in cache", chunk.GetID())
		} else {
			if uploadSize > 0 {
				...
				util.LOG_DEBUG("CHUNK_EXIST", "Skipped chunk %s in the storage", chunk.GetID())
			}
		}

		uploadedModifiedFileSize := atomic.AddInt64(&uploadedModifiedFileSize, int64(chunkSize))

		if (util.IsTracing() || showStatistics) && totalModifiedFileSize > 0 {
			now := time.Now().Unix()
			...
			util.LOG_INFO("UPLOAD_PROGRESS", "%s chunk %d size %d, %sB/s %s %.1f%%", action, chunkIndex,
				chunkSize, util.PrettySize(speed), util.PrettyTime(remainingTime), percentage/10)
		}
	...
	uploadChunkFunc := func(chunk *Chunk) {
		...
		if found {
			if time.Now().Unix()-lastUploadingTime > keepUploadAlive {
				util.LOG_INFO("UPLOAD_KEEPALIVE", "Skip chunk cache to keep connection alive")
				found = false
			}
		}
		...
		if chunkIndex == chunkToFail {
			util.LOG_ERROR("SNAPSHOT_FAIL", "Artificially fail the chunk %d for testing purposes", chunkToFail)
		}
	}
	...
	for i := range localEntryList.ModifiedEntries {
		...
		util.LOG_TRACE("PACK_START", "Packing %s", entry.Path)
		fullPath := joinPath(shadowTop, entry.Path)
		file, err := os.OpenFile(fullPath, os.O_RDONLY, 0)
		if err != nil {
			util.LOG_WARN("OPEN_FAILURE", "Failed to open file for reading: %v", err)
			skippedFiles = append(skippedFiles, entry.Path)
			continue
		}
		entry.Size, entry.Hash = fileChunkMaker.AddData(file, uploadChunkFunc)
		if !showStatistics || util.IsTracing() || util.RunInBackground {
			util.LOG_INFO("PACK_END", "Packed %s (%d)", entry.Path, entry.Size)
		}
		file.Close()
	...
	if showStatistics && !util.RunInBackground {
		for _, entry := range localEntryList.ModifiedEntries {
			if entry.Size < 0 {
				continue
			}
			util.LOG_INFO("UPLOAD_FILE", "Uploaded %s (%d)", entry.Path, entry.Size)
		}
	}

	for _, dir := range skippedDirectories {
		util.LOG_WARN("SKIP_DIRECTORY", "Subdirectory %s cannot be listed", dir)
	}

	for _, file := range skippedFiles {
		util.LOG_WARN("SKIP_FILE", "File %s cannot be opened", file)
	}

	if !manager.config.dryRun {
		manager.SnapshotManager.CleanSnapshotCache(localSnapshot, nil)
	}
	util.LOG_INFO("BACKUP_END", "Backup for %s at revision %d completed", top, localSnapshot.Revision)
	...
	if showStatistics {
		...
		util.LOG_INFO("BACKUP_STATS", "Files: %d total, %s bytes; %d new, %s bytes",
			localEntryList.NumberOfEntries-int64(len(skippedFiles)),
			util.PrettyNumber(preservedFileSize+uploadedFileSize),
			len(localEntryList.ModifiedEntries), util.PrettyNumber(uploadedFileSize))

		util.LOG_INFO("BACKUP_STATS", "File chunks: %d total, %s bytes; %d new, %s bytes, %s bytes uploaded",
			len(localSnapshot.ChunkHashes), util.PrettyNumber(totalFileChunkLength),
			numberOfNewFileChunks, util.PrettyNumber(totalUploadedFileChunkLength),
			util.PrettyNumber(totalUploadedFileChunkBytes))

		util.LOG_INFO("BACKUP_STATS", "Metadata chunks: %d total, %s bytes; %d new, %s bytes, %s bytes uploaded",
			totalMetadataChunks, util.PrettyNumber(totalMetadataChunkLength),
			numberOfNewMetadataChunks, util.PrettyNumber(totalUploadedMetadataChunkLength),
			util.PrettyNumber(totalUploadedMetadataChunkBytes))

		util.LOG_INFO("BACKUP_STATS", "All chunks: %d total, %s bytes; %d new, %s bytes, %s bytes uploaded",
			len(localSnapshot.ChunkHashes)+totalMetadataChunks,
			util.PrettyNumber(totalFileChunkLength+totalMetadataChunkLength),
			int(numberOfNewFileChunks)+numberOfNewMetadataChunks,
			util.PrettyNumber(totalUploadedFileChunkLength+totalUploadedMetadataChunkLength),
			util.PrettyNumber(totalUploadedFileChunkBytes+totalUploadedMetadataChunkBytes))

		now := time.Now().Unix()
		if now == startTime {
			now = startTime + 1
		}
		util.LOG_INFO("BACKUP_STATS", "Total running time: %s", util.PrettyTime(now-startTime))
	}
	...
	if len(skipped) > 0 {
		...
		util.LOG_WARN("BACKUP_SKIPPED", skipped)
	}
...
func (manager *BackupManager) Restore(top string, revision int, inPlace bool, quickMode bool, threads int, overwrite bool,
	...
	util.LOG_DEBUG("RESTORE_PARAMETERS", "top: %s, revision: %d, in-place: %t, quick: %t, delete: %t",
		top, revision, inPlace, quickMode, deleteMode)

	if !strings.HasPrefix(util.GetDuplicacyPreferencePath(), top) {
		util.LOG_INFO("RESTORE_INPLACE", "Forcing in-place mode with a non-default preference path")
		inPlace = true
	}

	if len(patterns) > 0 {
		for _, pattern := range patterns {
			util.LOG_TRACE("RESTORE_PATTERN", "%s", pattern)
		}
	}
	...
	if os.IsNotExist(err) {
		...
		if err != nil {
			util.LOG_ERROR("RESTORE_MKDIR", "Can't create the directory to be restored: %v", err)
			return 0
		}
	}

	err = os.Mkdir(path.Join(top, util.DUPLICACY_DIRECTORY), 0744)
	if err != nil && !os.IsExist(err) {
		util.LOG_ERROR("RESTORE_MKDIR", "Failed to create the preference directory: %v", err)
		return 0
	}
	...
	util.LOG_INFO("RESTORE_INDEXING", "Indexing %s", top)
	go func() {
		// List local files
		defer util.CatchLogException()
	...
	for remoteEntry := range remoteListingChannel {
		...
		if len(patterns) > 0 && !util.MatchPath(remoteEntry.Path, patterns) {
			continue
		}
		...
		if compareResult == 0 {
			if quickMode && localEntry.IsFile() && localEntry.IsSameAs(remoteEntry) {
				util.LOG_TRACE("RESTORE_SKIP", "File %s unchanged (by size and timestamp)", localEntry.Path)
		...
		if remoteEntry.IsLink() {
			...
			if err != nil {
				util.LOG_ERROR("RESTORE_SYMLINK", "Can't create symlink %s: %v", remoteEntry.Path, err)
				return 0
			}
			remoteEntry.RestoreMetadata(fullPath, nil, setOwner)
			util.LOG_TRACE("DOWNLOAD_DONE", "Symlink %s updated", remoteEntry.Path)
		} else if remoteEntry.IsDir() {

			stat, err := os.Stat(fullPath)

			if err == nil && !stat.IsDir() {
				util.LOG_ERROR("RESTORE_NOTDIR", "The path %s is not a directory", fullPath)
				return 0
			}
			...
			if os.IsNotExist(err) {
				...
				if err != nil && !os.IsExist(err) {
					util.LOG_ERROR("RESTORE_MKDIR", "%v", err)
					return 0
				}
			}
	...
	util.LOG_INFO("RESTORE_START", "Restoring %s to revision %d", top, revision)
	...
	for _, file := range fileEntries {
		...
		if stat != nil {
			if quickMode {
				if file.IsSameAsFileInfo(stat) {
					util.LOG_TRACE("RESTORE_SKIP", "File %s unchanged (by size and timestamp)", file.Path)
			...
			if file.Size == 0 && file.IsSameAsFileInfo(stat) {
				util.LOG_TRACE("RESTORE_SKIP", "File %s unchanged (size 0)", file.Path)
				skippedFileSize += file.Size
				skippedFileCount++
				continue
			}
			...
			if err != nil {
				util.LOG_ERROR("DOWNLOAD_MKDIR", "Failed to create directory: %v", err)
			}
		}
		...
		if file.Size == 0 {
			...
			if err != nil {
				util.LOG_ERROR("DOWNLOAD_OPEN", "Failed to create empty file: %v", err)
				return 0
			}
			newFile.Close()
			...
			if !showStatistics {
				util.LOG_INFO("DOWNLOAD_DONE", "Downloaded %s (0)", file.Path)
		...
		if err != nil {
			...
			failedFileCount++
			util.LOG_WARN("DOWNLOAD_FAIL", "Failed to restore %s: %v", file.Path, err)
			continue
		}
	...
	if deleteMode && len(patterns) == 0 {
		...
		for i := range extraFiles {
			...
			os.Remove(fullPath)
			util.LOG_INFO("RESTORE_DELETE", "Deleted %s", file)
		}
	}
	...
	if showStatistics {
		for _, file := range downloadedFiles {
			util.LOG_INFO("DOWNLOAD_DONE", "Downloaded %s (%d)", file.Path, file.Size)
		}
	}
	...
	util.LOG_INFO("RESTORE_END", "Restored %s to revision %d", top, revision)
	if showStatistics {
		util.LOG_INFO("RESTORE_STATS", "Files: %d total, %s bytes", len(fileEntries), util.PrettySize(totalFileSize))
		util.LOG_INFO("RESTORE_STATS", "Downloaded %d file, %s bytes, %d chunks",
			len(downloadedFiles), util.PrettySize(downloadedFileSize), chunkDownloader.numberOfDownloadedChunks)
		util.LOG_INFO("RESTORE_STATS", "Skipped %d file, %s bytes", skippedFileCount, util.PrettySize(skippedFileSize))
	}
	...
	util.LOG_INFO("RESTORE_STATS", "Total running time: %s", util.PrettyTime(runningTime))
...
func (encoder *fileEncoder) NextFile() (io.Reader, bool) {
	...
	if err != nil {
		util.LOG_FATAL("SNAPSHOT_ENCODE", "Failed to encode file %s: %v", encoder.files[encoder.currentIndex].Path, err)
		return nil, false
	}
...
func (manager *BackupManager) UploadSnapshot(chunkOperator *ChunkOperator, top string, snapshot *Snapshot,
	...
	uploadCompletionFunc := func(chunk *Chunk, chunkIndex int, inCache bool, chunkSize int, uploadSize int) {
		if inCache {
			util.LOG_DEBUG("CHUNK_CACHE", "Skipped metadata chunk %s in cache", chunk.GetID())
		} else {
			if uploadSize > 0 {
				...
				util.LOG_DEBUG("CHUNK_EXIST", "Skipped metadata chunk %s in the storage", chunk.GetID())
			}
		}
	...
	uploadEntryInfoFunc := func(entry *Entry) error {
		...
		if err != nil {
			util.LOG_ERROR("SNAPSHOT_UPLOAD", "Metadata for %s can't be encoded: %v", entry.Path, err)
			return err
		}
	...
	if err != nil {
		util.LOG_ERROR("SNAPSHOT_UPLOAD", "The file list contains an error: %v", err)
		return 0, 0, 0, 0
	}
	...
	for _, sequenceType := range []string{"chunks", "lengths"} {
		...
		if err != nil {
			util.LOG_ERROR("SNAPSHOT_MARSHAL", "Failed to encode the %s in the snapshot %s: %v",
	...
	if _, found := os.LookupEnv("DUPLICACY_FAIL_SNAPSHOT"); found {
		util.LOG_ERROR("SNAPSHOT_FAIL", "Artificially fail the backup for testing purposes")
		return 0, 0, 0, 0
	}

	description, err := snapshot.MarshalJSON()
	if err != nil {
		util.LOG_ERROR("SNAPSHOT_MARSHAL", "Failed to encode the snapshot %s: %v", manager.snapshotID, err)
		return int64(0), 0, int64(0), int64(0)
	}
...
func (manager *BackupManager) RestoreFile(chunkDownloader *ChunkDownloader, chunkMaker *ChunkMaker, entry *Entry, top string, inPlace bool, overwrite bool,
	...
	util.LOG_TRACE("DOWNLOAD_START", "Downloading %s", entry.Path)
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			if inPlace && entry.Size > 100*1024*1024 && runtime.GOOS != "darwin" {
				...
				if err != nil {
					util.LOG_ERROR("DOWNLOAD_CREATE", "Failed to create the file %s for in-place writing: %v", fullPath, err)
					return false, nil
				}
				...
				if err != nil {
					util.LOG_ERROR("DOWNLOAD_CREATE", "Failed to resize the initial file %s for in-place writing: %v", fullPath, err)
					return false, nil
				}
				_, err = existingFile.Write([]byte("\x00\x00")[:n])
				if err != nil {
					util.LOG_ERROR("DOWNLOAD_CREATE", "Failed to initialize the sparse file %s for in-place writing: %v", fullPath, err)
					return false, nil
				}
				existingFile.Close()
				existingFile, err = os.Open(fullPath)
				if err != nil {
					util.LOG_ERROR("DOWNLOAD_OPEN", "Can't reopen the initial file just created: %v", err)
					return false, nil
				}
				isNewFile = true
			}
		} else {
			util.LOG_TRACE("DOWNLOAD_OPEN", "Can't open the existing file: %v", err)
		}
	}
		...
		if inPlace {
			...
			for i := entry.StartChunk; i <= entry.EndChunk+1; i++ {
				...
				if isSkipped {
					...
					if err != nil {
						util.LOG_ERROR("DOWNLOAD_SEEK", "Failed to seek to offset %d: %v", offset, err)
					}
					isSkipped = false
				}
				...
				for count < chunkSize {
					...
					if err != nil {
						util.LOG_ERROR("DOWNLOAD_SPLIT", "Failed to read existing file: %v", err)
						return false, nil
					}
				}
				if count > 0 {
			...
			if fileHash == entry.Hash && fileHash != "" {
				util.LOG_TRACE("DOWNLOAD_SKIP", "File %s unchanged (by hash)", entry.Path)
				return false, nil
			}

			// fileHash != entry.Hash, warn/error depending on -overwrite option
			if !overwrite && !isNewFile {
				util.LOG_WERROR(allowFailures, "DOWNLOAD_OVERWRITE",
	...
	if inPlace {
		...
		util.LOG_TRACE("DOWNLOAD_INPLACE", "Updating %s in place", fullPath)

		if existingFile == nil {
			// Create an empty file
			existingFile, err = os.OpenFile(fullPath, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0600)
			if err != nil {
				util.LOG_ERROR("DOWNLOAD_CREATE", "Failed to create the file %s for in-place writing", fullPath)
			}
		} else {
			...
			if err != nil {
				util.LOG_ERROR("DOWNLOAD_OPEN", "Failed to open the file %s for in-place writing", fullPath)
				return false, nil
			}
		}
		...
		for i := entry.StartChunk; i <= entry.EndChunk; i++ {
			...
			_, err = existingFile.Seek(offset, 0)
			if err != nil {
				util.LOG_ERROR("DOWNLOAD_SEEK", "Failed to set the offset to %d for file %s: %v", offset, fullPath, err)
				return false, nil
			}
			...
			if existingOffset == offset && start == 0 && j < len(existingChunks) &&
				...
				if err != nil {
					util.LOG_ERROR("DOWNLOAD_READ", "Failed to read the existing chunk %s: %v", hash, err)
					return false, nil
				}
				if util.IsDebugging() {
					util.LOG_DEBUG("DOWNLOAD_UNCHANGED", "Chunk %s is unchanged", manager.config.GetChunkIDFromHash(hash))
				}
			} else {
				...
				if err != nil {
					util.LOG_ERROR("DOWNLOAD_WRITE", "Failed to write to the file: %v", err)
					return false, nil
				}
		...
		if err = existingFile.Truncate(offset); err != nil {
			util.LOG_ERROR("DOWNLOAD_TRUNCATE", "Failed to truncate the file at %d: %v", offset, err)
			return false, nil
		}
		...
		if hash != entry.Hash && hash != "" && entry.Hash != "" && !strings.HasPrefix(entry.Hash, "#") {
			util.LOG_WERROR(allowFailures, "DOWNLOAD_HASH", "File %s has a mismatched hash: %s instead of %s (in-place)",
		...
		if err != nil {
			util.LOG_ERROR("DOWNLOAD_OPEN", "Failed to open file for writing: %v", err)
			return false, nil
		}
		...
		for i := entry.StartChunk; i <= entry.EndChunk; i++ {
			...
			if existingFile != nil {
				if offset, ok := offsetMap[hash]; ok {
					...
					if err == nil {
						...
						if util.IsDebugging() {
							util.LOG_DEBUG("DOWNLOAD_LOCAL_COPY", "Local copy for chunk %s is available",
			...
			if err != nil {
				util.LOG_ERROR("DOWNLOAD_WRITE", "Failed to write file: %v", err)
				return false, nil
			}
		...
		hash := hex.EncodeToString(hasher.Sum(nil))
		if hash != entry.Hash && hash != "" && entry.Hash != "" && !strings.HasPrefix(entry.Hash, "#") {
			util.LOG_WERROR(allowFailures, "DOWNLOAD_HASH", "File %s has a mismatched hash: %s instead of %s",
		...
		if err != nil && !os.IsNotExist(err) {
			util.LOG_ERROR("DOWNLOAD_REMOVE", "Failed to remove the old file: %v", err)
			return false, nil
		}

		err = os.Rename(temporaryPath, fullPath)
		if err != nil {
			util.LOG_ERROR("DOWNLOAD_RENAME", "Failed to rename the file %s to %s: %v", temporaryPath, fullPath, err)
			return false, nil
		}
	}

	if !showStatistics {
		util.LOG_INFO("DOWNLOAD_DONE", "Downloaded %s (%d)", entry.Path, entry.Size)
	}
	return true, nil
}
...
func (manager *BackupManager) CopySnapshots(otherManager *BackupManager, snapshotID string,
	...
	if !manager.config.IsCompatibleWith(otherManager.config) {
		util.LOG_ERROR("CONFIG_INCOMPATIBLE", "Two storages are not compatible for the copy operation")
		return false
	}

	if otherManager.config.DataShards != 0 && otherManager.config.ParityShards != 0 {
		util.LOG_INFO("BACKUP_ERASURECODING", "Erasure coding is enabled for the destination storage with %d data shards and %d parity shards",
	...
	if otherManager.config.rsaPublicKey != nil && len(otherManager.config.FileKey) > 0 {
		util.LOG_INFO("BACKUP_KEY", "RSA encryption is enabled for the destination")
	}

	if snapshotID == "" && len(revisionsToBeCopied) > 0 {
		util.LOG_ERROR("SNAPSHOT_ERROR", "You must specify the snapshot id when one or more revisions are specified.")
		return false
	}
	...
	if snapshotID == "" {
		...
		if err != nil {
			util.LOG_ERROR("COPY_LIST", "Failed to list all snapshot ids: %v", err)
			return false
		}
	} else {
	...
	for _, id := range snapshotIDs {
		...
		for _, revision := range revisions {
			...
			if err != nil {
				util.LOG_ERROR("SNAPSHOT_INFO", "Failed to check if there is a snapshot %s at revision %d: %v",
					id, revision, err)
				return false
			}

			if exist {
				util.LOG_INFO("SNAPSHOT_EXIST", "Snapshot %s at revision %d already exists at the destination storage",
					id, revision)
				revisionMap[id][revision] = false
				continue
			}
	...
	if len(snapshots) == 0 {
		util.LOG_INFO("SNAPSHOT_COPY", "Nothing to copy, all snapshot revisions exist at the destination.")
		return true
	}
	...
	for _, snapshot := range snapshots {
		...
		util.LOG_TRACE("SNAPSHOT_COPY", "Copying snapshot %s at revision %d", snapshot.ID, snapshot.Revision)
	...
	for i, otherChunkID := range otherChunkFiles {
		...
		if otherChunkSizes[i] == 0 {
			util.LOG_DEBUG("SNAPSHOT_COPY", "Chunk %s has length = 0", otherChunkID)
			continue
		}
		otherChunks[otherChunkID] = false
	}

	util.LOG_DEBUG("SNAPSHOT_COPY", "Found %d chunks on destination storage", len(otherChunks))
	...
	util.LOG_INFO("SNAPSHOT_COPY", "Chunks to copy: %d, to skip: %d, total: %d", len(chunksToCopy), len(chunks)-len(chunksToCopy), len(chunks))
	...
	chunkUploader.UploadCompletionFunc = func(chunk *Chunk, chunkIndex int, skipped bool, chunkSize int, uploadSize int) {
		...
		util.LOG_INFO("COPY_PROGRESS", "%s chunk %s (%d/%d) %sB/s %s %.1f%%",
			action, chunk.GetID(), chunkIndex+1, len(chunksToCopy),
			util.PrettySize(speed), util.PrettyTime(remainingTime), percentage)
	...
	for i, chunkHash := range chunksToCopy {
		...
		util.LOG_DEBUG("SNAPSHOT_COPY", "Copying chunk %s to %s", chunkID, newChunkID)
	}

	chunkDownloader.Stop()
	chunkUploader.Stop()

	util.LOG_INFO("SNAPSHOT_COPY", "Copied %d new chunks and skipped %d existing chunks", copiedChunks, len(chunks)-copiedChunks)
	...
	for _, snapshot := range snapshots {
		...
		util.LOG_INFO("SNAPSHOT_COPY", "Copied snapshot %s at revision %d", snapshot.ID, snapshot.Revision)
	}
