...
func benchmarkSplit(reader *bytes.Reader, fileSize int64, chunkSize int, compression bool, encryption bool, annotation string) {
	...
	chunkFunc := func(chunk *Chunk) {
		if compression {
			...
			if err != nil {
				util.LOG_ERROR("BENCHMARK_ENCRYPT", "Failed to encrypt the chunk: %v", err)
			}
		}
	...
	util.LOG_INFO("BENCHMARK_SPLIT", "Split %s bytes into %d chunks %s in %.2fs: %s/s", util.PrettySize(fileSize), numberOfChunks, annotation,
		runningTime, util.PrettySize(speed))
...
func Benchmark(localDirectory string, storage backend.Storage, fileSize int64, chunkSize int, chunkCount int, uploadThreads int, downloadThreads int) bool {
	...
	util.LOG_INFO("BENCHMARK_GENERATE", "Generating %s byte random data in memory", util.PrettySize(fileSize))
	data := make([]byte, fileSize)
	_, err := rand.Read(data)
	if err != nil {
		util.LOG_ERROR("BENCHMARK_RAND", "Failed to generate random data: %v", err)
		return false
	}

	startTime := float64(time.Now().UnixNano()) / 1e9
	util.LOG_INFO("BENCHMARK_WRITE", "Writing random data to local disk")
	err = ioutil.WriteFile(filename, data, 0600)
	if err != nil {
		util.LOG_ERROR("BENCHMARK_WRITE", "Failed to write the random data: %v", err)
		return false
	}

	runningTime := float64(time.Now().UnixNano())/1e9 - startTime
	speed := int64(float64(fileSize) / runningTime)
	util.LOG_INFO("BENCHMARK_WRITE", "Wrote %s bytes in %.2fs: %s/s", util.PrettySize(fileSize), runningTime, util.PrettySize(speed))

	startTime = float64(time.Now().UnixNano()) / 1e9
	util.LOG_INFO("BENCHMARK_READ", "Reading the random data from local disk")
	file, err := os.Open(filename)
	if err != nil {
		util.LOG_ERROR("BENCHMARK_OPEN", "Failed to open the random data file: %v", err)
		return false
	}
	...
	if err != io.EOF {
		util.LOG_ERROR("BENCHMARK_OPEN", "Failed to read the random data file: %v", err)
		return false
	}
	file.Close()
	runningTime = float64(time.Now().UnixNano())/1e9 - startTime
	speed = int64(float64(fileSize) / runningTime)
	util.LOG_INFO("BENCHMARK_READ", "Read %s bytes in %.2fs: %s/s", util.PrettySize(fileSize), runningTime, util.PrettySize(speed))
	...
	if err != nil {
		util.LOG_ERROR("BENCHMARK_LIST", "Failed to list the benchmark directory: %v", err)
		return false
	}
	...
	if len(existingChunks) > 0 {
		util.LOG_INFO("BENCHMARK_DELETE", "Deleting %d temporary files from previous benchmark runs", len(existingChunks))
	...
	util.LOG_INFO("BENCHMARK_GENERATE", "Generating %d chunks", chunkCount)
	for i := 0; i < chunkCount; i++ {
	...
	benchmarkRun(uploadThreads, chunkCount, func(threadIndex int, chunkIndex int) {
		...
		if err != nil {
			util.LOG_ERROR("BENCHMARK_UPLOAD", "Failed to upload the chunk: %v", err)
			return
		}
	})
	...
	util.LOG_INFO("BENCHMARK_UPLOAD", "Uploaded %s bytes in %.2fs: %s/s", util.PrettySize(int64(chunkSize*chunkCount)), runningTime, util.PrettySize(speed))
	...
	benchmarkRun(downloadThreads, chunkCount, func(threadIndex int, chunkIndex int) {
		...
		if err != nil {
			util.LOG_ERROR("BENCHMARK_DOWNLOAD", "Failed to download the chunk: %v", err)
			return
		}
		...
		if hash != chunkHashes[chunkIndex] {
			util.LOG_WARN("BENCHMARK_HASH", "Chunk %d has mismatched hashes: %s != %s", chunkIndex, chunkHashes[chunkIndex], hash)
			hashError = true
		}
	...
	runningTime = float64(time.Now().UnixNano())/1e9 - startTime
	speed = int64(float64(chunkSize*chunkCount) / runningTime)
	util.LOG_INFO("BENCHMARK_DOWNLOAD", "Downloaded %s bytes in %.2fs: %s/s", util.PrettySize(int64(chunkSize*chunkCount)), runningTime, util.PrettySize(speed))

	if !hashError {
		benchmarkRun(uploadThreads, chunkCount, func(threadIndex int, chunkIndex int) {
			storage.DeleteFile(threadIndex, fmt.Sprintf("benchmark/chunk%d", chunkIndex))
		})
		util.LOG_INFO("BENCHMARK_DELETE", "Deleted %d temporary files from the storage", chunkCount)
	}
