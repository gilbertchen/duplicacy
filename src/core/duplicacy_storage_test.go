...
func loadStorage(localStoragePath string, threads int) (backend.Storage, error) {

	if *testStorageName == "" || *testStorageName == "file" {
		storage, err := backend.CreateFileStorage(localStoragePath, false, threads)
		if storage != nil {
	...
	config["localStoragePath"] = localStoragePath
	return backend.StorageGet(*testStorageName, config, threads)

}

func cleanStorage(storage backend.Storage) {
	...
	util.LOG_INFO("STORAGE_LIST", "Listing snapshots in the storage")
	for len(directories) > 0 {
		...
		if err != nil {
			util.LOG_ERROR("STORAGE_LIST", "Failed to list the directory %s: %v", dir, err)
			return
		}
	...
	util.LOG_INFO("STORAGE_DELETE", "Deleting %d snapshots in the storage", len(snapshots))
...
func listChunks(storage backend.Storage) (chunks []string) {
	...
	for len(directories) > 0 {
		...
		files, _, err := storage.ListFiles(0, dir)
		if err != nil {
			util.LOG_ERROR("CHUNK_LIST", "Failed to list the directory %s: %v", dir, err)
			return nil
		}
...
func moveChunk(t *testing.T, storage backend.Storage, chunkID string, isFossil bool, delay int) {
...
func TestStorage(t *testing.T) {
	...
	util.LOG_INFO("STORAGE_TEST", "storage: %s", *testStorageName)
	...
	if _, ok := storage.(*backend.HubicStorage); ok {
		delay = 2
	}
	...
	for i := 0; i < numberOfFiles; i++ {
		...
		util.LOG_INFO("STORAGE_CHUNK", "Uploaded chunk: %s, size: %d", filePath, len(content))
	}

	util.LOG_INFO("STORAGE_FOSSIL", "Making %s a fossil", chunks[0])
	moveChunk(t, storage, chunks[0], false, delay)
	util.LOG_INFO("STORAGE_FOSSIL", "Making %s a chunk", chunks[0])
	...
	for _, chunkID := range chunks {
		...
		if err != nil {
			...
			util.LOG_INFO("STORAGE_CHUNK", "Downloaded chunk: %s, size: %d", filePath, chunk.GetLength())
		}
	...
	util.LOG_INFO("STORAGE_FOSSIL", "Making %s a fossil", chunks[1])
...
func TestCleanStorage(t *testing.T) {
	...
	for len(directories) > 0 {
		...
		util.LOG_INFO("LIST_FILES", "Listing %s", dir)
		...
		for _, file := range files {
			if len(file) > 0 && file[len(file)-1] == '/' {
				...
				util.LOG_INFO("DELETE_FILE", "Deleted file %s", file)
			}
		}
	}

	storage.DeleteFile(0, "config")
	util.LOG_INFO("DELETE_FILE", "Deleted config")

	files, _, err := storage.ListFiles(0, "chunks/")
	for _, file := range files {
		if len(file) > 0 && file[len(file)-1] != '/' {
			util.LOG_DEBUG("FILE_EXIST", "File %s exists after deletion", file)
		}
	}
