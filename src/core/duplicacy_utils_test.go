...
func TestMatchPattern(t *testing.T) {
	...
	for _, data := range DATA {
		if util.MatchPattern(data.text, data.pattern) != data.matched {
...
func TestRateLimit(t *testing.T) {
	...
	expectedRate := 10
	rateLimiter := util.CreateRateLimitedReader(content, expectedRate)
	...
	startTime = time.Now()
	n, err = util.RateLimitedCopy(ioutil.Discard, bytes.NewBuffer(content), expectedRate)
	if err != nil {
