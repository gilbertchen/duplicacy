...
func (entry *Entry) RestoreMetadata(fullPath string, fileInfo *os.FileInfo, setOwner bool) bool {
	...
	if fileInfo == nil {
		...
		fileInfo = &stat
		if err != nil {
			util.LOG_ERROR("RESTORE_STAT", "Failed to retrieve the file info: %v", err)
			return false
		}
	}
	...
	if !entry.IsLink() && (*fileInfo).Mode()&fileModeMask != entry.GetPermissions() {
		...
		if err != nil {
			util.LOG_ERROR("RESTORE_CHMOD", "Failed to set the file permissions: %v", err)
			return false
		}
	}
	...
	if !entry.IsLink() && (*fileInfo).ModTime().Unix() != entry.Time {
		...
		if err != nil {
			util.LOG_ERROR("RESTORE_CHTIME", "Failed to set the modification time: %v", err)
			return false
		}
	}
...
func ListEntries(top string, path string, patterns []string, nobackupFile string, excludeByAttribute bool, listingChannel chan *Entry) (directoryList []*Entry,
	...
	util.LOG_DEBUG("LIST_ENTRIES", "Listing %s", path)
	...
	if nobackupFile != "" {
		...
		if ii < len(files) && files[ii].Name() == nobackupFile {
			util.LOG_DEBUG("LIST_NOBACKUP", "%s is excluded due to nobackup file", path)
	...
	for _, f := range files {
		if f.Name() == util.DUPLICACY_DIRECTORY {
			continue
		}
		entry := CreateEntryFromFileInfo(f, normalizedPath)
		if len(patterns) > 0 && !util.MatchPath(entry.Path, patterns) {
			continue
		}
		if entry.IsLink() {
			isRegular := false
			...
			if err != nil {
				util.LOG_WARN("LIST_LINK", "Failed to read the symlink %s: %v", entry.Path, err)
			...
			if isRegular {
				...
				if err != nil {
					util.LOG_WARN("LIST_LINK", "Failed to read the symlink: %v", err)
					skippedFiles = append(skippedFiles, entry.Path)
					continue
				}
				...
				if len(patterns) > 0 && !util.MatchPath(newEntry.Path, patterns) {
					continue
				}
				entry = newEntry
			}
		...
		if excludeByAttribute && entry.Attributes != nil && excludedByAttribute(*entry.Attributes) {
			util.LOG_DEBUG("LIST_EXCLUDE", "%s is excluded by attribute", entry.Path)
			continue
		}

		if f.Mode()&(os.ModeNamedPipe|os.ModeSocket|os.ModeDevice) != 0 {
			util.LOG_WARN("LIST_SKIP", "Skipped non-regular file %s", entry.Path)
			skippedFiles = append(skippedFiles, entry.Path)
			continue
		}
