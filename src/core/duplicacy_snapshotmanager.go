...
func (collection *FossilCollection) IsDeletable(isStrongConsistent bool, ignoredIDs []string,
	...
	for snapshotID, snapshotList := range allSnapshots {
		...
		if ignored {
			util.LOG_INFO("SNAPSHOT_NOT_CONSIDERED", "Ignored snapshot %s", snapshotID)
			continue
		}
		...
		for _, snapshot := range snapshotList {
			...
			if snapshot.EndTime > collection.EndTime+int64(extraTime) {
				...
				util.LOG_TRACE("SNAPSHOT_UNDELETABLE",
		...
		if !hasNewSnapshot[hostID] {
			util.LOG_TRACE("SNAPSHOT_NO_NEW", "No new snapshot from %s since the fossil collection step", snapshotID)
		}
	...
	for hostID, value := range hasNewSnapshot {
		if value == false {
			...
			if lastSnapshotTime[hostID] > 0 && lastSnapshotTime[hostID] < time.Now().Unix()-maxSnapshotRunningTime*secondsInDay {
				util.LOG_INFO("SNAPSHOT_INACTIVE", "Ignore snapshot %s whose last revision was created more than %d days ago",
...
type SnapshotManager struct {
	...
	fileChunk     *Chunk
	snapshotCache *backend.FileStorage

	chunkOperator *ChunkOperator
}

// CreateSnapshotManager creates a snapshot manager
func CreateSnapshotManager(config *Config, storage backend.Storage) *SnapshotManager {
...
func (manager *SnapshotManager) DownloadSnapshot(snapshotID string, revision int) *Snapshot {
	...
	if err != nil {
		util.LOG_ERROR("SNAPSHOT_INFO", "Failed to get the information on the snapshot %s at revision %d: %v",
			snapshotID, revision, err)
		return nil
	}

	if !exist {
		util.LOG_ERROR("SNAPSHOT_NOT_EXIST", "Snapshot %s at revision %d does not exist", snapshotID, revision)
		return nil
	...
	if err != nil {
		util.LOG_ERROR("SNAPSHOT_PARSE", "Failed to parse the snapshot %s at revision %d: %v", snapshotID, revision, err)
		return nil
	}
...
func (manager *SnapshotManager) DownloadSnapshotSequence(snapshot *Snapshot, sequenceType string) bool {
	...
	if len(content) == 0 {
		util.LOG_ERROR("SNAPSHOT_PARSE", "Failed to load %s specified in the snapshot %s at revision %d",
	...
	if err != nil {
		util.LOG_ERROR("SNAPSHOT_PARSE", "Failed to load %s specified in the snapshot %s at revision %d: %v",
...
func (manager *SnapshotManager) CleanSnapshotCache(latestSnapshot *Snapshot, allSnapshots map[string][]*Snapshot) bool {
	...
	if allSnapshots == nil {
		...
		if err != nil {
			util.LOG_ERROR("SNAPSHOT_CLEAN", "Failed to list the snapshot cache: %v", err)
			return false
		}
	...
	for _, snapshotFile := range allCachedSnapshots {
		...
		if _, found := allSnapshotFiles[snapshotFile]; !found {
			util.LOG_DEBUG("SNAPSHOT_CLEAN", "Delete cached snapshot file %s not found in the storage", snapshotFile)
		...
		if err != nil {
			util.LOG_WARN("SNAPSHOT_CACHE", "Failed to read the cached snapshot file: %v", err)
			continue
		}
		...
		if err != nil {
			util.LOG_ERROR("SNAPSHOT_CACHE", "Failed to parse the cached snapshot file %s: %v", snapshotFile, err)
			continue
		}
		...
		if !isComplete {
			util.LOG_DEBUG("SNAPSHOT_CLEAN", "Delete cached snapshot file %s with nonexistent chunks", snapshotFile)
			manager.snapshotCache.DeleteFile(0, path.Join("snapshots", snapshotFile))
			continue
		}
		...
		for _, chunkHash := range cachedSnapshot.ChunkSequence {
			chunkID := manager.config.GetChunkIDFromHash(chunkHash)
			util.LOG_DEBUG("SNAPSHOT_CLEAN", "Snapshot %s revision %d needs chunk %s", cachedSnapshot.ID, cachedSnapshot.Revision, chunkID)
			chunks[chunkID] = true
		}
	}
	...
	for _, file := range allFiles {
		if len(file) > 0 && file[len(file)-1] != '/' {
			...
			if _, found := chunks[chunkID]; !found {
				util.LOG_DEBUG("SNAPSHOT_CLEAN", "Delete chunk %s from the snapshot cache", chunkID)
				err := manager.snapshotCache.DeleteFile(0, path.Join("chunks", file))
				if err != nil {
					util.LOG_WARN("SNAPSHOT_CLEAN", "Failed to remove the chunk %s from the snapshot cache: %v",
						file, err)
				}
			}
		}
...
func (manager *SnapshotManager) ListSnapshotIDs() (snapshotIDs []string, err error) {
	...
	util.LOG_TRACE("SNAPSHOT_LIST_IDS", "Listing all snapshot ids")
...
func (manager *SnapshotManager) ListSnapshotRevisions(snapshotID string) (revisions []int, err error) {
	...
	util.LOG_TRACE("SNAPSHOT_LIST_REVISIONS", "Listing revisions for snapshot %s", snapshotID)
	...
	if err != nil {
		util.LOG_WARN("SNAPSHOT_CACHE_DIR", "Failed to create the snapshot cache directory %s: %v", snapshotDir, err)
	}
...
func (manager *SnapshotManager) downloadLatestSnapshot(snapshotID string) (remote *Snapshot) {
	...
	util.LOG_TRACE("SNAPSHOT_DOWNLOAD_LATEST", "Downloading latest revision for snapshot %s", snapshotID)

	revisions, err := manager.ListSnapshotRevisions(snapshotID)

	if err != nil {
		util.LOG_ERROR("SNAPSHOT_LIST", "Failed to list the revisions of the snapshot %s: %v", snapshotID, err)
		return nil
	}
...
func (manager *SnapshotManager) ListAllFiles(storage backend.Storage, top string) (allFiles []string, allSizes []int64) {
	...
	for len(directories) > 0 {
		...
		util.LOG_TRACE("LIST_FILES", "Listing %s", dir)

		files, sizes, err := storage.ListFiles(0, dir)
		if err != nil {
			util.LOG_ERROR("LIST_FILES", "Failed to list the directory %s: %v", dir, err)
			return nil, nil
		}
...
func (manager *SnapshotManager) GetSnapshotChunks(snapshot *Snapshot, keepChunkHashes bool) (chunks []string) {
	...
	if len(snapshot.ChunkHashes) == 0 {
		...
		if err != nil {
			util.LOG_ERROR("SNAPSHOT_CHUNK", "Failed to load chunks for snapshot %s at revision %d: %v",
...
func (manager *SnapshotManager) ListSnapshots(snapshotID string, revisionsToList []int, tag string,
	...
	util.LOG_DEBUG("LIST_PARAMETERS", "id: %s, revisions: %v, tag: %s, showFiles: %t, showChunks: %t",
	...
	if snapshotID == "" {
		...
		if err != nil {
			util.LOG_ERROR("SNAPSHOT_LIST", "Failed to list all snapshots: %v", err)
			return 0
		}
	} else {
	...
	for _, snapshotID = range snapshotIDs {
		...
		if len(revisions) == 0 {
			revisions, err = manager.ListSnapshotRevisions(snapshotID)
			if err != nil {
				util.LOG_ERROR("SNAPSHOT_LIST", "Failed to list all revisions for snapshot %s: %v", snapshotID, err)
				return 0
			}
		}
		...
		for _, revision := range revisions {
			...
			util.LOG_INFO("SNAPSHOT_INFO", "Snapshot %s revision %d created at %s %s%s",
			...
			if showFiles {
				...
				if snapshot.NumberOfFiles > 0 {
					util.LOG_INFO("SNAPSHOT_STATS", "Files: %d", snapshot.NumberOfFiles)
				}
				...
				snapshot.ListRemoteFiles(manager.config, manager.chunkOperator, func(file *Entry) bool {
					if file.IsFile() {
						util.LOG_INFO("SNAPSHOT_FILE", "%s", file.String(maxSizeDigits))
					}
					return true
				})

				metaChunks := len(snapshot.FileSequence) + len(snapshot.ChunkSequence) + len(snapshot.LengthSequence)
				util.LOG_INFO("SNAPSHOT_STATS", "Total size: %d, file chunks: %d, metadata chunks: %d", totalFileSize, lastChunk+1, metaChunks)
			}

			if showChunks {
				for _, chunkID := range manager.GetSnapshotChunks(snapshot, false) {
					util.LOG_INFO("SNAPSHOT_CHUNKS", "chunk: %s", chunkID)
				}
			}
...
func (manager *SnapshotManager) CheckSnapshots(snapshotID string, revisionsToCheck []int, tag string, showStatistics bool, showTabular bool,
	...
	util.LOG_DEBUG("LIST_PARAMETERS", "id: %s, revisions: %v, tag: %s, showStatistics: %t, showTabular: %t, checkFiles: %t, searchFossils: %t, resurrect: %t",
	...
	util.LOG_INFO("SNAPSHOT_CHECK", "Listing all chunks")
	...
	for i, chunk := range allChunks {
		...
		if allSizes[i] == 0 && !strings.HasSuffix(chunk, ".tmp") {
			util.LOG_WARN("SNAPSHOT_CHECK", "Chunk %s has a size of 0", chunk)
			emptyChunks++
		}
	}
	...
	util.LOG_INFO("SNAPSHOT_CHECK", "%d snapshots and %d revisions", len(snapshotMap), totalRevisions)

	var totalChunkSize int64
	for _, size := range chunkSizeMap {
		totalChunkSize += size
	}
	util.LOG_INFO("SNAPSHOT_CHECK", "Total chunk size is %s in %d chunks", util.PrettyNumber(totalChunkSize), len(chunkSizeMap))
	...
	for snapshotID = range snapshotMap {
		...
		for _, snapshot := range snapshotMap[snapshotID] {
			...
			for chunkID := range chunks {
				...
				if !found {
					...
					if missingChunks < 100 {
						...
						if err != nil {
							util.LOG_WARN("SNAPSHOT_VALIDATE", "Failed to check the existence of chunk %s: %v",
								chunkID, err)
						} else if exist {
							util.LOG_INFO("SNAPSHOT_VALIDATE", "Chunk %s is confirmed to exist", chunkID)
							continue
						}
					}
					...
					if !searchFossils {
						missingChunks += 1
						util.LOG_WARN("SNAPSHOT_VALIDATE",
					...
					if err != nil {
						util.LOG_ERROR("SNAPSHOT_VALIDATE", "Failed to check the existence of fossil %s: %v",
							chunkID, err)
						return false
					}
					...
					if resurrect {
						...
						util.LOG_WARN("SNAPSHOT_FOSSIL", "Chunk %s referenced by snapshot %s at revision %d "+
			...
			if missingChunks > 0 {
				util.LOG_WARN("SNAPSHOT_CHECK", "Some chunks referenced by snapshot %s at revision %d are missing",
					snapshotID, snapshot.Revision)
				totalMissingChunks += missingChunks
			} else {
				util.LOG_INFO("SNAPSHOT_CHECK", "All chunks referenced by snapshot %s at revision %d exist",
					snapshotID, snapshot.Revision)
			}
		}
	...
	if totalMissingChunks > 0 {
		util.LOG_ERROR("SNAPSHOT_CHECK", "Some chunks referenced by some snapshots do not exist in the storage")
		return false
	}

	if emptyChunks > 0 {
		util.LOG_ERROR("SNAPSHOT_CHECK", "%d chunks have a size of 0", emptyChunks)
		return false
	}
	...
	if err != nil {
		if !os.IsNotExist(err) {
			util.LOG_WARN("SNAPSHOT_VERIFY", "Failed to load the file containing verified chunks: %v", err)
		}
	} else {
		err = json.Unmarshal(manager.fileChunk.GetBytes(), &verifiedChunks)
		if err != nil {
			util.LOG_WARN("SNAPSHOT_VERIFY", "Failed to parse the file containing verified chunks: %v", err)
		}
	}
	...
	saveVerifiedChunks := func() {
		...
		if len(verifiedChunks) > numberOfVerifiedChunks {
			...
			if err != nil {
				util.LOG_WARN("SNAPSHOT_VERIFY", "Failed to create a json file for the set of verified chunks: %v", err)
			} else {
				err = manager.snapshotCache.UploadFile(0, verifiedChunksFile, description)
				if err != nil {
					util.LOG_WARN("SNAPSHOT_VERIFY", "Failed to save the verified chunks file: %v", err)
				} else {
					util.LOG_INFO("SNAPSHOT_VERIFY", "Added %d chunks to the list of verified chunks", len(verifiedChunks)-numberOfVerifiedChunks)
					numberOfVerifiedChunks = len(verifiedChunks)
				}
			}
		}
	...
	util.RunAtError = saveVerifiedChunks

	util.LOG_INFO("SNAPSHOT_VERIFY", "Verifying %d chunks", len(*allChunkHashes))
	...
	if skippedChunks > 0 {
		util.LOG_INFO("SNAPSHOT_VERIFY", "Skipped %d chunks that have already been verified before", skippedChunks)
	}
		...
		go func() {
			...
			for {
				...
				if !chunk.isBroken {
					...
					util.LOG_INFO("VERIFY_PROGRESS", "Verified chunk %s (%d/%d), %sB/s %s %.1f%%",
						chunkID, downloadedChunks, totalChunks, util.PrettySize(speed), util.PrettyTime(remainingTime), percentage)
				}
	...
	if manager.chunkOperator.NumberOfFailedChunks > 0 {
		util.LOG_ERROR("SNAPSHOT_VERIFY", "%d out of %d chunks are corrupted", manager.chunkOperator.NumberOfFailedChunks, len(*allChunkHashes))
	} else {
		util.LOG_INFO("SNAPSHOT_VERIFY", "All %d chunks have been successfully verified", len(*allChunkHashes))
	}
	return true
}
	...
	for snapshotID, snapshotList := range snapshotMap {
		...
		for _, snapshot := range snapshotList {
			...
			if snapshot.FileSize != 0 && snapshot.NumberOfFiles != 0 {
				files = fmt.Sprintf("%d files (%s bytes), ", snapshot.NumberOfFiles, util.PrettyNumber(snapshot.FileSize))
			}
			util.LOG_INFO("SNAPSHOT_CHECK", "Snapshot %s at revision %d: %s%s total chunk bytes, %s unique chunk bytes",
				snapshot.ID, snapshot.Revision, files, util.PrettyNumber(totalChunkSize), util.PrettyNumber(uniqueChunkSize))
		}
		...
		util.LOG_INFO("SNAPSHOT_CHECK", "Snapshot %s all revisions: %s total chunk bytes, %s unique chunk bytes",
			snapshotID, util.PrettyNumber(totalChunkSize), util.PrettyNumber(uniqueChunkSize))
	}
}
...
func (manager *SnapshotManager) ShowStatisticsTabular(snapshotMap map[string][]*Snapshot, chunkSizeMap map[string]int64, chunkUniqueMap map[string]bool,
	...
	for snapshotID, snapshotList := range snapshotMap {
		...
		for _, snapshot := range snapshotList {
			...
			files := " \t "
			if snapshot.FileSize != 0 && snapshot.NumberOfFiles != 0 {
				files = fmt.Sprintf("%d \t%s", snapshot.NumberOfFiles, util.PrettyNumber(snapshot.FileSize))
			}
			creationTime := time.Unix(snapshot.StartTime, 0).Format("2006-01-02 15:04")
			fmt.Fprintln(tableWriter, fmt.Sprintf(
				"%s \t%d \t@ %s %5s \t%s \t%d \t%s \t%d \t%s \t%d \t%s \t",
				snapshotID, snapshot.Revision, creationTime, snapshot.Options, files, totalChunkCount, util.PrettyNumber(totalChunkSize), uniqueChunkCount, util.PrettyNumber(uniqueChunkSize), newChunkCount, util.PrettyNumber(newChunkSize)))
		}
		...
		fmt.Fprintln(tableWriter, fmt.Sprintf(
			"%s \tall \t \t \t \t%d \t%s \t%d \t%s \t \t \t",
			snapshotID, totalChunkCount, util.PrettyNumber(totalChunkSize), uniqueChunkCount, util.PrettyNumber(uniqueChunkSize)))
	}
	tableWriter.Flush()
	util.LOG_INFO("SNAPSHOT_CHECK", tableBuffer.String())
}
...
func (manager *SnapshotManager) PrintSnapshot(snapshot *Snapshot) bool {
	...
	if err != nil {
		util.LOG_ERROR("SNAPSHOT_PRINT", "Failed to marshal the snapshot %s at revision %d: %v",
			snapshot.ID, snapshot.Revision, err)
		return false
	}
...
func (manager *SnapshotManager) VerifySnapshot(snapshot *Snapshot) bool {
	...
	if err != nil {
		util.LOG_ERROR("SNAPSHOT_CHECK", "Snapshot %s at revision %d has an error: %v",
			snapshot.ID, snapshot.Revision, err)
		return false
	}
	...
	for _, file := range files {
		...
		util.LOG_TRACE("SNAPSHOT_VERIFY", "%s", file.Path)
	}
	...
	if corruptedFiles > 0 {
		util.LOG_WARN("SNAPSHOT_VERIFY", "Snapshot %s at revision %d contains %d corrupted files",
			snapshot.ID, snapshot.Revision, corruptedFiles)
		return false
	} else {
		util.LOG_INFO("SNAPSHOT_VERIFY", "All files in snapshot %s at revision %d have been successfully verified",
			snapshot.ID, snapshot.Revision)
		return true
	}
}
...
func (manager *SnapshotManager) RetrieveFile(snapshot *Snapshot, file *Entry, lastChunk **Chunk, output func([]byte)) bool {
	...
	if strings.ToLower(fileHash) != strings.ToLower(file.Hash) && !SkipFileHash {
		util.LOG_WARN("SNAPSHOT_HASH", "File %s has mismatched hashes: %s vs %s", file.Path, file.Hash, fileHash)
		return false
	}
	return true
}
...
func (manager *SnapshotManager) FindFile(snapshot *Snapshot, filePath string, suppressError bool) *Entry {
	...
	if !suppressError {
		util.LOG_ERROR("SNAPSHOT_FIND", "No file %s found in snapshot %s at revision %d",
...
func (manager *SnapshotManager) PrintFile(snapshotID string, revision int, path string) bool {
	...
	util.LOG_DEBUG("PRINT_PARAMETERS", "id: %s, revision: %d, path: %s", snapshotID, revision, path)
	...
	if revision <= 0 {
		...
		if snapshot == nil {
			util.LOG_ERROR("SNAPSHOT_PRINT", "No previous snapshot %s is not found", snapshotID)
			return false
		}
	} else {
		snapshot = manager.DownloadSnapshot(snapshotID, revision)
	...
	if !manager.RetrieveFile(snapshot, file, nil, func(chunk []byte) {
		...
		util.LOG_ERROR("SNAPSHOT_RETRIEVE", "File %s is corrupted in snapshot %s at revision %d",
...
func (manager *SnapshotManager) Diff(top string, snapshotID string, revisions []int,
	...
	util.LOG_DEBUG("DIFF_PARAMETERS", "top: %s, id: %s, revision: %v, path: %s, compareByHash: %t",
	...
	if len(revisions) < 1 {
		...
		if leftSnapshot == nil {
			util.LOG_ERROR("SNAPSHOT_DIFF", "No previous snapshot %s is not found", snapshotID)
			return false
		}
	} else {
	...
	if len(filePath) > 0 {
		...
		if !manager.RetrieveFile(leftSnapshot, manager.FindFile(leftSnapshot, filePath, false), nil, func(content []byte) {
			...
			util.LOG_ERROR("SNAPSHOT_DIFF", "File %s is corrupted in snapshot %s at revision %d",
		...
		if rightSnapshot != nil {
			...
			if err != nil {
				util.LOG_ERROR("SNAPSHOT_DIFF", "Failed to read %s from the repository: %v", filePath, err)
				return false
			}
		}
	...
	for i < len(leftSnapshotFiles) || j < len(rightSnapshotFiles) {
		...
		if i >= len(leftSnapshotFiles) {
			if rightSnapshotFiles[j].IsFile() {
				util.LOG_INFO("SNAPSHOT_DIFF", "+ %s", rightSnapshotFiles[j].String(maxSizeDigits))
			}
			j++
		} else if j >= len(rightSnapshotFiles) {
			if leftSnapshotFiles[i].IsFile() {
				util.LOG_INFO("SNAPSHOT_DIFF", "- %s", leftSnapshotFiles[i].String(maxSizeDigits))
			}
			i++
		} else {
			...
			if c < 0 {
				util.LOG_INFO("SNAPSHOT_DIFF", "- %s", left.String(maxSizeDigits))
				i++
			} else if c > 0 {
				util.LOG_INFO("SNAPSHOT_DIFF", "+ %s", right.String(maxSizeDigits))
				j++
			} else {
				same := false
				if rightSnapshot.Revision == 0 {
...
func (manager *SnapshotManager) ShowHistory(top string, snapshotID string, revisions []int,
	...
	util.LOG_DEBUG("HISTORY_PARAMETERS", "top: %s, id: %s, revisions: %v, path: %s, showLocalHash: %t",
	...
	for _, revision := range revisions {
		...
		if file != nil {
			...
			util.LOG_INFO("SNAPSHOT_HISTORY", "%7d: %s%s", revision, file.String(15), modifiedFlag)
			lastVersion = file
		} else {
			util.LOG_INFO("SNAPSHOT_HISTORY", "%7d:", revision)
		}
	...
	if stat != nil {
		...
		util.LOG_INFO("SNAPSHOT_HISTORY", "current: %s%s", localFile.String(15), modifiedFlag)
	} else {
		util.LOG_INFO("SNAPSHOT_HISTORY", "current:")
	}
...
func (manager *SnapshotManager) resurrectChunk(fossilPath string, chunkID string) bool {
	...
	if err != nil {
		util.LOG_ERROR("CHUNK_FIND", "Failed to locate the path for the chunk %s: %v", chunkID, err)
		return false
	}

	if exist {
		manager.storage.DeleteFile(0, fossilPath)
		util.LOG_INFO("FOSSIL_RECREATE", "The chunk %s already exists", chunkID)
	} else {
		err := manager.storage.MoveFile(0, fossilPath, chunkPath)
		if err != nil {
			util.LOG_ERROR("FOSSIL_RESURRECT", "Failed to resurrect the chunk %s from the fossil %s: %v",
				chunkID, fossilPath, err)
			return false
		} else {
			util.LOG_INFO("FOSSIL_RESURRECT", "The chunk %s has been resurrected", fossilPath)
		}
	}
	return true
}
...
func (manager *SnapshotManager) PruneSnapshots(selfID string, snapshotID string, revisionsToBeDeleted []int,
	...
	util.LOG_DEBUG("DELETE_PARAMETERS",
	...
	if len(revisionsToBeDeleted) > 0 && (len(tags) > 0 || len(retentions) > 0) {
		util.LOG_WARN("DELETE_OPTIONS", "Tags or retention policy will be ignored if at least one revision is specified")
	}
	...
	prefPath := util.GetDuplicacyPreferencePath()
	logDir := path.Join(prefPath, "logs")
	err := os.MkdirAll(logDir, 0700)
	if err != nil {
		util.LOG_ERROR("util.LOG_DIR", "Could not open log directory %s: %v", logDir, err)
	}
	...
	if err != nil {
		util.LOG_ERROR("util.LOG_FILE", "Could not open log file %s: %v", logFileName, err)
	}
	...
	defer func() {
		if logFile != nil {
			cerr := logFile.Close()
			if cerr != nil {
				util.LOG_WARN("util.LOG_FILE", "Could not close log file %s: %v", logFileName, cerr)
			}
		}
	}()
	...
	if len(revisionsToBeDeleted) == 0 && len(retentions) > 0 {
		...
		for _, retention := range retentions {
			...
			if matched == nil {
				util.LOG_ERROR("RETENTION_INVALID", "Invalid retention policy: %s", retention)
				return false
			}
		...
		if len(retentionPolicies) == 0 {
			util.LOG_ERROR("RETENTION_INVALID", "Invalid retention policy: %v", retentions)
			return false
		}
		...
		for i, policy := range retentionPolicies {
			if i == 0 || policy.Age < retentionPolicies[i-1].Age {
				if policy.Interval == 0 {
					util.LOG_INFO("RETENTION_POLICY", "Keep no snapshots older than %d days", policy.Age)
				} else {
					util.LOG_INFO("RETENTION_POLICY", "Keep 1 snapshot every %d day(s) if older than %d day(s)",
	...
	for _, id := range snapshotIDs {
		...
		if err != nil {
			util.LOG_ERROR("SNAPSHOT_LIST", "Failed to list all revisions for snapshot %s: %v", id, err)
			return false
		}
	...
	if err != nil {
		util.LOG_ERROR("FOSSIL_COLLECT", "Failed to create collection directory %s: %v", collectionDir, err)
		return false
	}

	collections, _, err := manager.snapshotCache.ListFiles(0, collectionDir)
	if err != nil {
		util.LOG_ERROR("FOSSIL_COLLECT", "Failed to list fossil collection files for dir %s: %v", collectionDir, err)
		return false
	}
	...
	for _, collectionName := range collections {
		...
		if err != nil {
			util.LOG_ERROR("FOSSIL_COLLECT", "Failed to read the fossil collection file %s: %v", collectionFile, err)
			return false
		}
		...
		if err != nil {
			util.LOG_ERROR("FOSSIL_COLLECT", "Failed to load the fossil collection file %s: %v", collectionFile, err)
			return false
		}
			...
			for _, revision := range revisionList {
				for _, snapshot := range allSnapshots[snapshotID] {
					if revision == snapshot.Revision {
						util.LOG_INFO("FOSSIL_GHOSTSNAPSHOT", "Snapshot %s revision %d should have been deleted already", snapshotID, revision)
						exist = true
					}
				}
			}
		...
		if exist {
			...
			if err != nil {
				util.LOG_WARN("FOSSIL_FILE", "Failed to remove the fossil collection file %s: %v", collectionFile, err)
			} else {
				util.LOG_INFO("FOSSIL_IGNORE", "The fossil collection file %s has been ignored due to ghost snapshots", collectionFile)
			}
			continue
		}
		...
		util.LOG_INFO("FOSSIL_COLLECT", "Fossil collection %s found", collectionName)
		...
		if isDeletable || exclusive {
			...
			util.LOG_INFO("FOSSIL_DELETABLE", "Fossils from collection %s is eligible for deletion", collectionName)
			...
			for _, newSnapshot := range newSnapshots {
				...
				util.LOG_INFO("PRUNE_NEWSNAPSHOT", "Snapshot %s revision %d was created after collection %s", newSnapshot.ID, newSnapshot.Revision, collectionName)
			...
			for _, fossil := range collection.Fossils {
				...
				if _, found := newChunks[chunk]; found {
					...
					if dryRun {
						util.LOG_INFO("FOSSIL_RESURRECT", "Fossil %s would be resurrected", chunk)
						continue
					}
					...
					if dryRun {
						util.LOG_INFO("FOSSIL_DELETE", "The chunk %s would be permanently removed", chunk)
					} else {
			...
			for _, temporary := range collection.Temporaries {
				if dryRun {
					util.LOG_INFO("TEMPORARY_DELETE", "The temporary file %s would be deleted", temporary)
				} else {
			...
			util.LOG_TRACE("FOSSIL_END", "Finished processing fossil collection %s", collectionName)
		} else {
			util.LOG_INFO("FOSSIL_POSTPONE",
		...
		if len(revisionsToBeDeleted) > 0 {
			...
			for _, snapshot := range snapshots {
				if _, found := revisionMap[snapshot.Revision]; found {
					util.LOG_DEBUG("SNAPSHOT_DELETE", "Snapshot %s at revision %d to be deleted - specified in command",
						snapshot.ID, snapshot.Revision)
					snapshot.Flag = true
					toBeDeleted++
				}
			...
			for j, snapshot := range snapshots {
				...
				if i < len(retentionPolicies) {
					if retentionPolicies[i].Interval == 0 {
						...
						util.LOG_DEBUG("SNAPSHOT_DELETE", "Snapshot %s at revision %d to be deleted - older than %d days",
						...
						util.LOG_DEBUG("SNAPSHOT_DELETE", "Snapshot %s at revision %d to be deleted - older than %d days, less than %d days from previous",
	...
	if toBeDeleted == 0 && !exhaustive {
		util.LOG_INFO("SNAPSHOT_NONE", "No snapshot to delete")
		return false
	}
	...
	if !collection.IsEmpty() && !dryRun && !exclusive {
		...
		if err != nil {
			util.LOG_ERROR("FOSSIL_COLLECT", "Failed to create a json file for the fossil collection: %v", err)
			return false
		}

		err = manager.snapshotCache.UploadFile(0, collectionFile, description)
		if err != nil {
			util.LOG_ERROR("FOSSIL_COLLECT", "Failed to save the fossil collection file %s: %v", collectionFile, err)
			return false
		}

		util.LOG_INFO("FOSSIL_COLLECT", "Fossil collection %d saved", collectionNumber)
	...
	for _, snapshots := range allSnapshots {
		for _, snapshot := range snapshots {
			...
			if err != nil {
				util.LOG_ERROR("SNAPSHOT_DELETE", "Failed to delete the snapshot %s at revision %d: %v",
					snapshot.ID, snapshot.Revision, err)
				return false
			}
			util.LOG_INFO("SNAPSHOT_DELETE", "The snapshot %s at revision %d has been removed",
				snapshot.ID, snapshot.Revision)
			err = manager.snapshotCache.DeleteFile(0, snapshotPath)
			if err != nil {
				util.LOG_WARN("SNAPSHOT_DELETE", "The cached snapshot %s at revision %d could not be removed: %v",
					snapshot.ID, snapshot.Revision, err)
...
func (manager *SnapshotManager) pruneSnapshotsNonExhaustive(allSnapshots map[string][]*Snapshot, collection *FossilCollection, logFile io.Writer, dryRun, exclusive bool) bool {
	...
	for _, snapshots := range allSnapshots {
		...
		if len(snapshots) > 0 {
			...
			if latest.Flag && !exclusive {
				util.LOG_ERROR("SNAPSHOT_DELETE",
		...
		for _, snapshot := range snapshots {
			...
			util.LOG_INFO("SNAPSHOT_DELETE", "Deleting snapshot %s at revision %d", snapshot.ID, snapshot.Revision)
	...
	for chunk, value := range targetChunks {
		...
		if dryRun {
			util.LOG_INFO("CHUNK_UNREFERENCED", "Found unreferenced chunk %s", chunk)
			continue
		}
...
func (manager *SnapshotManager) pruneSnapshotsExhaustive(referencedFossils map[string]bool, allSnapshots map[string][]*Snapshot, collection *FossilCollection, logFile io.Writer, dryRun, exclusive bool) bool {
	...
	for _, file := range allFiles {
		...
		if strings.HasSuffix(file, ".tmp") {
			...
			if dryRun {
				util.LOG_INFO("CHUNK_TEMPORARY", "Found temporary file %s", file)
				continue
			}
			...
			if _, found := referencedFossils[file]; !found {
				...
				if _, found := referencedChunks[chunk]; found {
					...
					if dryRun {
						util.LOG_INFO("FOSSIL_REFERENCED", "Found referenced fossil %s", file)
						continue
					}
					...
					if dryRun {
						util.LOG_INFO("FOSSIL_UNREFERENCED", "Found unreferenced fossil %s", file)
						continue
					}
					...
					if exclusive {
						...
						util.LOG_DEBUG("FOSSIL_FIND", "Found unreferenced fossil %s", file)
					}
		...
		if !chunkRegex.MatchString(chunk) {
			util.LOG_WARN("CHUNK_UNKNOWN_FILE", "File %s is not a chunk", file)
			continue
		}
		...
		if value, found := referencedChunks[chunk]; !found {
			...
			if dryRun {
				util.LOG_INFO("CHUNK_REDUNDANT", "Found redundant chunk %s", chunk)
				continue
			}
			...
			util.LOG_DEBUG("CHUNK_KEEP", "Chunk %s is referenced", chunk)
		}
	}
...
func (manager *SnapshotManager) DownloadFile(path string, derivationKey string) (content []byte) {
	...
	if manager.storage.IsCacheNeeded() {
		...
		if err == nil && len(manager.fileChunk.GetBytes()) > 0 {
			util.LOG_DEBUG("DOWNLOAD_FILE_CACHE", "Loaded file %s from the snapshot cache", path)
	...
	if err != nil {
		util.LOG_ERROR("DOWNLOAD_FILE", "Failed to download the file %s: %v", path, err)
		return nil
	}
	...
	if err != nil {
		util.LOG_ERROR("DOWNLOAD_DECRYPT", "Failed to decrypt the file %s: %v", path, err)
		return nil
	}
	...
	if rewriteNeeded && manager.chunkOperator.rewriteChunks {
		...
		if err == nil {
			...
			if err != nil {
				util.LOG_WARN("DOWNLOAD_REWRITE", "Failed to re-uploaded the file %s: %v", path, err)
			} else {
				util.LOG_INFO("DOWNLOAD_REWRITE", "The file %s has been re-uploaded", path)
			}
		}
	...
	if err != nil {
		util.LOG_WARN("DOWNLOAD_FILE_CACHE", "Failed to add the file %s to the snapshot cache: %v", path, err)
	}

	util.LOG_DEBUG("DOWNLOAD_FILE", "Downloaded file %s", path)
...
func (manager *SnapshotManager) UploadFile(path string, derivationKey string, content []byte) bool {
	...
	if manager.storage.IsCacheNeeded() {
		err := manager.snapshotCache.UploadFile(0, path, manager.fileChunk.GetBytes())
		if err != nil {
			util.LOG_WARN("UPLOAD_CACHE", "Failed to cache the file %s: %v", path, err)
		} else {
			util.LOG_DEBUG("UPLOAD_FILE_CACHE", "Saved file %s to the snapshot cache", path)
		}
	}
	...
	if err != nil {
		util.LOG_ERROR("UPLOAD_File", "Failed to encrypt the file %s: %v", path, err)
		return false
	}

	err = manager.storage.UploadFile(0, path, manager.fileChunk.GetBytes())
	if err != nil {
		util.LOG_ERROR("UPLOAD_File", "Failed to upload the file %s: %v", path, err)
		return false
	}

	util.LOG_DEBUG("UPLOAD_FILE", "Uploaded file %s", path)
