...
func DeleteShadowCopy() {
	...
	if err != nil {
		util.LOG_WARN("VSS_DELETE", "Error while unmounting snapshot: %v", err)
		return
	}

	err = exec.Command("tmutil", "deletelocalsnapshots", snapshotDate).Run()
	if err != nil {
		util.LOG_WARN("VSS_DELETE", "Error while deleting local snapshot: %v", err)
		return
	}

	err = os.RemoveAll(snapshotPath)
	if err != nil {
		util.LOG_WARN("VSS_DELETE", "Error while deleting temporary mount directory: %v", err)
		return
	}

	util.LOG_INFO("VSS_DELETE", "Shadow copy unmounted and deleted at %s", snapshotPath)
...
func CreateShadowCopy(top string, shadowCopy bool, timeoutInSeconds int) (shadowTop string) {
	...
	if err != nil {
		util.LOG_ERROR("VSS_INIT", "Unable to determine filesystem of repository path")
		return top
	}
	if CharsToString(stat.Fstypename[:]) != "apfs" {
		util.LOG_WARN("VSS_INIT", "VSS requires APFS filesystem")
		return top
	}
	...
	if err != nil {
		util.LOG_ERROR("VSS_INIT", "Unable to get device ID of path: /")
		return top
	}
	deviceIdRepository, err := GetPathDeviceId(top)
	if err != nil {
		util.LOG_ERROR("VSS_INIT", "Unable to get device ID of path: %s", top)
		return top
	}
	if deviceIdLocal != deviceIdRepository {
		util.LOG_WARN("VSS_PATH", "VSS not supported for non-local repository path: %s", top)
		return top
	}
	...
	if err != nil {
		util.LOG_ERROR("VSS_CREATE", "Failed to create temporary mount directory")
		return top
	}
	...
	if err != nil {
		util.LOG_ERROR("VSS_CREATE", "Error while calling tmutil: %v", err)
		return top
	}
	...
	if matched == nil {
		util.LOG_ERROR("VSS_CREATE", "Snapshot creation failed: %s", tmutilOutput)
		return top
	}
	...
	if err != nil {
		util.LOG_ERROR("VSS_CREATE", "Error while calling 'tmutil listlocalsnapshots': %v", err)
		return top
	}
	...
	if len(matched) > 0 {
		...
		util.LOG_INFO("VSS_CREATE", "Can't find the snapshot name with 'tmutil listlocalsnapshots'; fallback to %s", snapshotName)
	}
	...
	if err != nil {
		util.LOG_ERROR("VSS_CREATE", "Error while mounting snapshot: %v", err)
		return top
	}

	util.LOG_INFO("VSS_DONE", "Shadow copy created and mounted at %s", snapshotPath)
