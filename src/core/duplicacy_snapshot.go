...
func (snapshot *Snapshot) ListLocalFiles(top string, nobackupFile string,
	...
	if filtersFile == "" {
		filtersFile = joinPath(util.GetDuplicacyPreferencePath(), "filters")
	}
	...
	for len(directories) > 0 {
		...
		if err != nil {
			if directory.Path == "" {
				util.LOG_ERROR("LIST_FAILURE", "Failed to list the repository root: %v", err)
				return
			}
			util.LOG_WARN("LIST_FAILURE", "Failed to list subdirectory %s: %v", directory.Path, err)
...
func (snapshot *Snapshot) ListRemoteFiles(config *Config, chunkOperator *ChunkOperator, entryOut func(*Entry) bool) {
	...
	if snapshot.Version == 0 || reader.GetFirstByte() == '[' {
		util.LOG_INFO("SNAPSHOT_VERSION", "snapshot %s at revision %d is encoded in an old version format", snapshot.ID, snapshot.Revision)
		files := make([]*Entry, 0)
		...
		_, err := decoder.Token()
		if err != nil {
			util.LOG_ERROR("SNAPSHOT_PARSE", "Failed to open the snapshot %s at revision %d: not a list of entries",
		...
		for decoder.More() {
			...
			if err != nil {
				util.LOG_ERROR("SNAPSHOT_PARSE", "Failed to load files specified in the snapshot %s at revision %d: %v",
		...
		for _, err := decoder.PeekCode(); err != io.EOF; _, err = decoder.PeekCode() {
			if err != nil {
				util.LOG_ERROR("SNAPSHOT_PARSE", "Failed to parse the snapshot %s at revision %d: %v",
					snapshot.ID, snapshot.Revision, err)
				return
			}
			var entry Entry
			err = decoder.Decode(&entry)
			if err != nil {
				util.LOG_ERROR("SNAPSHOT_PARSE", "Failed to load the snapshot %s at revision %d: %v",
					snapshot.ID, snapshot.Revision, err)
				return
			}
			...
			if err != nil {
				util.LOG_ERROR("SNAPSHOT_ENTRY", "Failed to load the snapshot %s at revision %d: %v",
					snapshot.ID, snapshot.Revision, err)
				return
			}
		...
		util.LOG_ERROR("SNAPSHOT_VERSION", "snapshot %s at revision %d is encoded in unsupported version %d format",
...
func AppendPattern(patterns []string, new_pattern string) (new_patterns []string) {
	for _, pattern := range patterns {
		if pattern == new_pattern {
			util.LOG_INFO("SNAPSHOT_FILTER", "Ignoring duplicate pattern: %s ...", new_pattern)
			return patterns
		}
	}
...
func ProcessFilters(filtersFile string) (patterns []string) {
	...
	if util.IsTracing() {
		for _, pattern := range patterns {
			util.LOG_TRACE("SNAPSHOT_PATTERN", "Pattern: %s", pattern)
		}
...
func ProcessFilterFile(patternFile string, includedFiles []string) (patterns []string) {
	for _, file := range includedFiles {
		if file == patternFile {
			...
			util.LOG_ERROR("SNAPSHOT_FILTER", "The filter file %s has already been included", patternFile)
			return patterns
		}
	}
	includedFiles = append(includedFiles, patternFile)
	util.LOG_INFO("SNAPSHOT_FILTER", "Parsing filter file %s", patternFile)
...
func ProcessFilterLines(patternFileLines []string, includedFiles []string) (patterns []string) {
	for _, pattern := range patternFileLines {
		...
		if util.IsUnspecifiedFilter(pattern) {
			pattern = "+" + pattern
		}

		if util.IsEmptyFilter(pattern) {
			continue
		}

		if strings.HasPrefix(pattern, "i:") || strings.HasPrefix(pattern, "e:") {
			valid, err := util.IsValidRegex(pattern[2:])
			if !valid || err != nil {
				util.LOG_ERROR("SNAPSHOT_FILTER", "Invalid regular expression encountered for filter: \"%s\", error: %v", pattern, err)
			}
		}
